...
:::note
For enterprise projects, these event notifications include [workflow runs](/reference/concepts/workflows.md#running-a-workflow). Additionally, notifications can be sent on workflow wait step events (e.g. continue, approve, and reject).
...
In order for the notification message hyperlink to properly redirect you to the change or workflow run, setup the `public` tenant in `OPSCHAIN_TENANT_BASE_URLS` environment variable.

e.g.

```text
OPSCHAIN_TENANT_BASE_URLS='{"public": "https://my_public_url.com"}'
```
...
- Email - after configuring the [mail server](/operations/notifications.md#email) in the notification channels
- Slack - after configuring the [Slack](/operations/notifications.md#slack) service url in the notification channels
- Teams - after configuring the [Teams](/operations/notifications.md#teams) service url in the notification channels
...
:::note
For [workflow runs](/reference/concepts/workflows.md#running-a-workflow), additional events include `on_continue` on a wait step, and `on_reject` and `on_approve` on an approval step.
