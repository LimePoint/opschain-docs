...
- `warn:git_remote:mountable_fetch_failed` — OpsChain could not refresh its copy of the Git remote before mirroring it into a runner pod for the [`git_clone` resource](/advanced/included-resource-types.md#opschain-git-clone). The pod is still started, so the mirror the step checks out from may be out of date, or missing altogether if the remote has never been fetched successfully. Throttled to one event per hour.
