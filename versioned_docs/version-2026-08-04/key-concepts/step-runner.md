...
The `.git` directory itself (the full commit history) is a separate, often larger contributor to `repo.tar`'s size. By default it is no longer included at all, since the default Dockerfile doesn't use it. If your custom Dockerfile relies on `.git` being present, enable the [`include_git_history`](/key-concepts/settings.md#include_git_history) setting.
...
A sample `step_context.json` file is available to view [here](/files/samples/2026-08-04/step_context.json).
