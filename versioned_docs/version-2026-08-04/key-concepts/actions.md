...
This precedence applies **at any depth**, not just to top-level actions: a nested `actions.rb` action (one reached via another action's [child steps](#child-steps)) takes precedence over a same-named MintModel action just as a top-level one does.
