...
Set the `stepEmoji` CLI configuration option to `false` to show text rather than emojis for the step status - see the [CLI configuration guide](/advanced/advanced-cli.md#complete-opschain-cli-configuration) for more details.
...
Install the CLI [dev dependencies](/advanced/advanced-cli.md#dev-subcommand-dependencies) and then run the command again.
...
If the configuration option [OPSCHAIN_BUILD_SERVICE_ROOTLESS](/setup/configuration/additional-settings.md#opschain_image_build_rootless) is set to true and the `opschain-build-service` pod is stuck in a 'pending' state during an upgrade, it may be that the `fuse-device-plugin-daemonset` daemonset is not running or requires a restart. This pod is responsible for making the required FUSE devices on the node, available to the `opschain-build-service` pod when running in rootless mode.
