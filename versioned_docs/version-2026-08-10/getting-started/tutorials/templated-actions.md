...
OpsChain will automatically fetch properties from the [OPSCHAIN_REPO_FOLDER](/setup/configuration/additional-settings.md#opschain_repo_folder) directory, where we can map our OpsChain structure to define properties for each structural level (project, environment, asset).
...
:::tip[Template folder properties]
You can also place property files (`.json`, `.yaml`, or `.toml`) directly in the template's folder - alongside its `actions.rb` - to ship a set of default properties with the template itself. See [template folder properties](/key-concepts/properties.md#template-folder-properties) for details.
:::
...
You can read more about template versions in the [asset templates guide](/getting-started/familiarisation/gui/projects/asset_templates.md#about-asset-template-versions).
:::
