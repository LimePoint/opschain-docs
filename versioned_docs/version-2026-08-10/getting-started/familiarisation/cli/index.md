...
| `--skip-steps` | | No | Glob pattern matching the full path of a step to skip - repeatable, not comma-separated |
| `--starting-step` | | No | For templated or MintModel changes, the full path of the step to begin execution at |
| `--old-mintmodel-id`<br/>`--new-mintmodel-id` | | No | Run a MintModel difference change on a templated node, diffing from the first MintModel to the second. Both must be given together, and cannot be combined with scheduling |
| `--auto-continue-wait-steps` | | No | Have the server continue any wait step the change hits, without you needing to wait or stay attached |
| `--wait-for-completion` | `-w` | No | Poll every 5 seconds until terminal state |
| `--show-logs` | | No | Stream logs in real-time (requires `-w`) |
| `--show-steps` | | No | Show a live-updating tree of the change's steps while waiting (requires `-w`, cannot be combined with `--show-logs`) |
| `--utc` | | No | Display timestamps in UTC |
| `--from-file` | | No | Load entire request from a JSON file |

### 11.3 Continuing a waiting change

A change that reaches a wait or input step pauses until it is continued.

```bash
# Continue a change with a single waiting step (auto-detected)
opschain changes continue b5bf89b6

# Record a message against the continue
opschain changes continue b5bf89b6 -m "Sanity checks done, ok to proceed"

# Continue a specific step, skipping the lookup
opschain changes continue b5bf89b6 --step afe3063d

# Continue every waiting step of the change
...
If the change has several waiting steps they are listed, and you must re-run with `--step` or `--all`. This command covers change steps only - workflow steps are not handled here.

### 11.4 Running an action across many assets

`changes execute` is the bulk sibling of `changes create` - it runs one action against every matching asset, creating one change per asset. `--assets` is required, and takes either a comma-separated list of codes or a quoted `'*'` for every asset in scope.

```bash
# Run 'deploy' on two assets in the dev environment
opschain changes execute -E dev --assets web,api -a deploy

# Fan out across an (environment x asset) matrix
opschain changes execute -E dev,test --assets '*' -a deploy

# Preview what would be created, without creating anything
opschain changes execute -E dev --assets '*' -a deploy --dry-run

# Wait for every created change and report their final statuses
opschain changes execute -E dev --assets '*' -a deploy -w
```

`-E`/`--environment` is optional. Omit it to target project-level assets - those that don't sit in an environment - where `'*'` means every project-level asset. Project-level rows show `(project)` in the output table, and carry an empty environment in JSON and YAML output.

Assets that don't exist or don't support the action are skipped and listed in the summary rather than failing the run. The `-t`, `--metadata`, `--property-overrides`, `--settings-overrides`, `--skip-steps`, `--starting-step`, `--build-without-cache` and `--auto-continue-wait-steps` flags all mirror `changes create` and apply to every created change.

### 11.5 Listing and filtering changes
...
# Notify people about the run
opschain workflows runs create --code deploy-app --version 2 \
  --notify-email ops@example.com --notify-ldap-group platform-team

# Notify only when the run errors or finishes
opschain workflows runs create --code deploy-app --version 2 \
  --notify-email ops@example.com --notify-event error,finish
...
Recipients are given with `--notify-email`, `--notify-user-id` and `--notify-ldap-group`, each repeatable or comma-separated. `--notify-event` selects which lifecycle events trigger a notification - `cancel`, `create`, `error`, `finish`, `start` or `success` - and defaults to all of them.
...
Event timestamps are shown in your local timezone. Add `--utc` to show them in UTC instead - this works on both `events list` and `events get`.
