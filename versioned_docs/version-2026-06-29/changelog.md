...
- It is now possible to skip specific steps when starting or retrying a change or workflow run. Set a `skip_steps` array of glob patterns on the change, workflow run, scheduled change, or scheduled workflow — any step whose identifier matches a pattern is automatically skipped at runtime (`full_path` is matched for change steps; MintModel steps are matched by their hierarchical step name as it appears in the step tree (e.g. `**/Install jdk Binaries`); `name` for workflow steps). See [skipping steps](/key-concepts/changes.md) for more information.
