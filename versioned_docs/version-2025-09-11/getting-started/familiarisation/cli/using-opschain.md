...
:::info
This example uses OpsChain properties for simplicity. OpsChain properties are stored encrypted at rest and are tracked for auditability, but it may be preferable to use [OpsChain secrets](/getting-started/tutorials/using-secrets-in-your-change.md) for sensitive information like AWS access keys.
:::
...
2. In contrast to the simple actions used throughout this guide, various [sample projects](/getting-started/tutorials/samples.md) are available to demonstrate how OpsChain can affect real change on local and cloud servers.
