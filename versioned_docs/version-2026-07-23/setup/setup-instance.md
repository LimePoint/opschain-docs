...
If `opschain-ca` itself is ever renewed (for example, if it's force-rotated, or its `duration`/`renewBefore` is changed by an OpsChain upgrade), you must re-extract it and restart K3s for the change to take effect:
