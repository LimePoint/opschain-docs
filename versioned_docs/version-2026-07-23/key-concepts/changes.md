...
When a change starts executing it enters the `queued` state. Changes and steps stay in the `queued` state while they are waiting for an OpsChain worker to start executing them (e.g. if all workers are already busy, a change will wait until a worker is available). A change or step may also wait here when the cluster has reached its [runner pod concurrency cap](/key-concepts/settings.md#runner-pod-concurrency-settings), starting once a running pod completes and frees capacity.
...
3. A retried change reconciles its settings against the current settings schema, which may have changed since the original change ran. This picks up any newly-added default settings and drops override settings that are no longer valid — the dropped settings are recorded in the change's events.
...
`starting_step` can only be supplied for a change that has a generated step tree, and must name a step within it.
