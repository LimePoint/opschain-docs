...
- `dry_run` — OpsChain evaluates whether it would grow or shrink the worker count and records the decision as an event, but never changes the worker count. This is the default, so autoscaling behaviour can be observed safely before it is relied upon.
- `active` — OpsChain grows or shrinks the worker count for real, as well as recording the same events.
