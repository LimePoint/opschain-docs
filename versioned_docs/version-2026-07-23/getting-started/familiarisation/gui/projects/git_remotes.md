...
5. (Optional, superusers only) If you are connecting to an SSH host that is not yet trusted, tick _Trust host_ to scan and trust the host's SSH key. See [trusting a new host](#trusting-a-new-host) for details.
6. Click the _Add git remote_ button. If successful, the new Git remote will appear on the Git remotes list of the project.
