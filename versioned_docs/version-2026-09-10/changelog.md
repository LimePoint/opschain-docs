...
`helm upgrade` scales `opschain-api-worker` to zero and waits for the work its pods are running to finish, before the new API applies the migrations and the workers return.

- Changes and workflow runs make no progress in between, so upgrade while little is in flight.
- A worker still busy after `rolloutGuard.drainTimeoutSeconds` — 30 minutes by default — aborts the upgrade. Let that work finish and upgrade again, or raise the timeout.
- In a [high availability deployment](/advanced/ha/index.md), only the site being upgraded is drained. Upgrade every site before letting work resume, otherwise a peer's workers are running against a schema newer than their own code.

The check stays in place for later upgrades, and skips the drain in a few seconds where the release carries no migration that needs it.
...
- Filtering changes, steps and activities by `status_code` now returns the records whose displayed status is the one you asked for. The filter worked the status out separately from the status shown on the record itself, and the two could disagree: `rejected` matched anything aborted while it had approvers, `pending` missed the records waiting to be restarted, and `waiting` missed a change whose outstanding steps were all waiting. See [API filtering & sorting](/advanced/api-filtering.md).
...
- Deleting a user now removes their authorisation policy assignments, so recreating an account of the same name does not restore its access. Assigning a policy to a username that neither OpsChain nor the LDAP directory holds is now refused with a message naming it, rather than stored and left waiting for that person to sign in; an LDAP account that has never signed in has its OpsChain user created so the assignment takes effect immediately.
...
- A resource that re-declares its available actions now applies the step name its own entries carry. The entry's `step_name:` was previously ignored, leaving the action with the step name the resource type gave it, or with none. `ignore_failure:` was already applied this way, so all of an entry's options now behave alike. See [defining resource types & resources](/key-concepts/actions.md#controller).
- A resource that narrows which of its controller's actions are available no longer loses the step name and options those actions were defined with. Listing an action by name in a resource's `available_actions` reset its `step_name`, `ignore_failure` and `reload_actions` to their defaults, and an entry that renamed one action blanked the description of that action for every resource of the same type defined after it. An entry now overrides only the options it states. See [overriding controller actions on a resource](/key-concepts/actions.md#overriding-controller-actions-on-a-resource).
...
- Filtering activities by `status_code` no longer returns every waiting workflow run when you ask for `waiting_for_approval`, or every aborted workflow run when you ask for `rejected`. The approval check behind both values was always true for a workflow run, so each of them matched on the underlying status alone.
- The activities list now reports `waiting` for a change paused on a wait or input step that has steps nested under it. Only a wait step with nothing beneath it counted as waiting, so such a change was listed as `running` and `status_code_eq=waiting` did not return it. The change's own status was right throughout. See [nesting child steps under a wait step](/key-concepts/actions.md#nesting-child-steps-under-a-wait-step).
...
- The steps marked to skip in the run change dialog are now cleared when you choose a different action. A selection made under one action was submitted against another, where those steps do not exist.
...
- Each cleanup job run now reports the filters that run applied, and a job's card summarises the filters set against each of its options. An agent image cleanup was captioned with the change and workflow run filters instead. See [reviewing what a cleanup job has removed](/getting-started/familiarisation/gui/data_cleanup.md#reviewing-what-a-cleanup-job-has-removed).
- The agent images option on a cleanup job now warns when it has no filter set, as every other option already did. Left blank, it removes every unreferenced agent image. See [what a cleanup job can remove](/getting-started/familiarisation/gui/data_cleanup.md#what-a-cleanup-job-can-remove).
...
- Commit file patterns can no longer be set on a scheduled change without **New commits only**, which is the trigger they filter. The combination was refused by the API when the change was saved. See [limiting a scheduled change to particular files](/getting-started/familiarisation/gui/scheduled_activities.md#limiting-a-scheduled-change-to-particular-files).
...
- An authorisation policy can now be assigned to a local user. The assignee was looked up in the LDAP directory whatever their account, so granting a policy to a locally created user failed saying the user could not be found.
...
- A cleanup job now reports how many agent images and job history rows it removed, and no longer purges the image of a build that has not finished, which could leave that agent unable to start. An invalid filter on the agent image purge is refused when the job is saved rather than when it runs, so a stored definition carrying one must be corrected before it can be saved again.
