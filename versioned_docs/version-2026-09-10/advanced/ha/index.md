...
An OpsChain **site** is one complete OpsChain deployment: a Kubernetes cluster running an API, its workers, and its own **database cluster**. A high availability deployment is several sites, kept in step by replicating one site's database cluster to the others.

Throughout the high availability guides _cluster_ means a site's database cluster — the `Cluster` resource `kubectl` reports and the name you set in `db.cnpg.clusterName`. _Site_ is used where something applies to the whole deployment rather than only its database.

### Distributed topology

OpsChain makes use of CNPG's high availability distributed topology pattern by default, providing an easy declarative addition and removal of sites from the topology, allowing you to scale and prepare for disaster recovery as necessary. When running with multiple sites, OpsChain databases provide protection against complete site or regional failures, manual failover between sites (declarative configuration change), and minimal data loss and downtime in case of a database node failure.

Since every site has details of the others in its configuration, it is possible to failover and failback between sites without having to rebuild a site's database cluster from scratch.

### Where work runs

Every site shares the same data, so any site's GUI or API can see and act on everything. What actually runs the work is pinned to one site:

- A [change](/key-concepts/changes.md) is pinned to the site that built its runner image, and every step of that change runs there. The change details page shows the site. Everytime a change is created or repeated, it may be picked up by any site in the topology that has workers available.
- An [agent](/getting-started/familiarisation/gui/projects/agents.md) is pinned to the site that started it, until it is stopped. A stop is sent to that site, so an agent can be started and stopped from any site.
- A project's [Git remotes](/getting-started/familiarisation/gui/projects/git_remotes.md) are checked out on each site's own storage, so every site fetches and keeps its own copy. A fetch at one site does not stand in for another's, each site's periodic fetching runs to its own schedule, and a remote that cannot be reached from one site backs off there alone. A site added to the topology fetches each remote once, the first time it is needed there.

Each site's Kubernetes resources — its [pods](/getting-started/familiarisation/gui/pods.md), [deployments](/getting-started/familiarisation/gui/deployments.md) and their events — can only be inspected from the site that holds them.

## Prerequisites

To run a high availability OpsChain setup, some conditions must be met:

- All sites must have network access to each other, including access to the database ports.
- Database credentials and encryption keys must be the same across all sites.
- Replication can only be setup from a single source site. Data conciliation (active-active mode) is not supported.
...
Within `connectionParameters`, the `user` value must always be defined and set to `streaming_replica`, `host` and `port` must match the other cluster's hostname and `db.cnpg.externalService.nodePort` respectively. The `dbname` and `sslmode` settings can be ommited.
...
- Follow the [replica cluster setup guide](/advanced/ha/replica-cluster-setup.md) to add a replica cluster to your topology, step by step.
- Learn how to perform [operations on a high availability OpsChain setup](/advanced/ha/operations.md), including failover and failback.
