...
| `git_remotes`      | The project's [Git remotes](/getting-started/familiarisation/gui/projects/git_remotes.md) the [`git_clone` resource](/advanced/resource-types/index.md#opschain-git-clone) may check out, as controlled by [`git_remote.mountable`](/key-concepts/settings.md#git_remotemountable)<br />Credentials are never included |
| `ssh_settings`     | The SSH liveness options OpsChain applies to its own Git operations — `connect_timeout`, `server_alive_interval` and `server_alive_count_max` — as controlled by [`git_remote.ssh`](/key-concepts/settings.md#git_remotesshconnect_timeout). The [`git_clone` resource](/advanced/resource-types/index.md#opschain-git-clone) applies them to a checkout from a `url`, so an unresponsive Git server cannot hang your step indefinitely |
...
import SampleContextValues from '/files/samples/2026-09-10/sample-context-values.md'
