...
### `api.certificateSecretName`

Default value: _none_

The [Kubernetes TLS secret](https://kubernetes.io/docs/concepts/configuration/secret/#tls-secrets) name containing a custom certificate to be used for the HTTPS listener. Setting it stops `cert-manager` issuing a certificate for the API, so the certificate you supply must cover the host name in [`api.hostName`](#envopschain_api_host_name-and-apihostname). [Learn more](/setup/configuration/tls/manual-cert-management.md#bringing-your-own-certificates).
...
OpsChain can optionally run a [CloudNative PostgreSQL (CNPG)](https://cloudnative-pg.io/) session-mode connection pooler in front of the database, reducing the per-connection setup cost (TCP/TLS handshake and authentication) under heavy connection churn. See the [database connection pooling guide](/advanced/database-connection-pooling.md) for when to use it, how to verify it, and how to roll it back.
...
:::note
Mutually exclusive with [`db.cnpg.pooler.enabled`](#dbcnpgpoolerenabled) - do not enable both at once, as the two settings resolve `PGHOST` differently.
:::
...
Docker Hub password/token to be used for accessing the OpsChain images on the external image registry. Rotate it in an upgrade of its own — see [rotating the Docker Hub credentials](#settings-managed-by-the-deployment).
...
Internally used hostname that needs to resolve to the Kubernetes node, but be different to the API hostname. This setting is not populated automatically and must be set explicitly, or the installation will fail. [Learn more](/setup/configuration/tls/index.md#image-registry-hostname).

#### `trow.trow.user`

Default value: _opschain_

Username for the internal image registry.

#### `trow.trow.password`

Default value: _none_

Password for the internal image registry.
...
Default value: _900_ (15 minutes)

How long the pulls may take before they are abandoned and the release is reported as failed. Keep this below the `--timeout` used for the [installation](/setup/installation.md) or [upgrade](/operations/upgrading.md).
...
The TLS port that Kong's proxy service publishes - how the cluster network, and containerd on every node, addresses the ingress. It is used to build the internal image registry references that Kubernetes pulls runner, agent and MintModel API images from, the matching entry in the image pull secret, and the API address handed to agent pods.
...
:::info[Not the port your users dial]
This is the port on the cluster network, which is not necessarily the port clients reach OpsChain on from outside. If you run a load balancer in front of the cluster that translates the port - for example it listens on 443 and forwards to the node on 17001 - this setting is the port the load balancer forwards *to* (17001 in that example). The externally dialled port only needs recording separately for the secret vault, as [`global.secretVaultExternalPort`](#globalsecretvaultexternalport).
:::
...
Default value: _none_ - follows `kong.proxy.tls.servicePort`, which is `443` when that is itself unset.

The port an external client dials to reach the OpsChain secret vault at [`global.secretVaultExternalHostName`](/setup/configuration/tls/index.md#secret-vault-hostname).
...
Set it when a load balancer in front of the cluster translates the port: if it accepts connections on 443 and forwards to Kong on 17001, then `global.secretVaultExternalPort` is `443` while [`OPSCHAIN_INGRESS_TLS_PORT`](#opschain_ingress_tls_port) stays `17001`.
...
:::warning[Changing this requires the secret vault replicas to be recycled]
...
Nothing verifies that the address can be reached, and a wrong value fails intermittently — only the requests that land on a standby replica are affected, and a single-replica secret vault never redirects at all — so the problem can lie dormant. See [secret vault requests fail with `no route to host`](/troubleshooting.md#secret-vault-requests-fail-with-no-route-to-host) for how to check the address in use and recycle the replicas safely.
:::

:::info[Previously `global.ingressTlsPort`]
This value was called `global.ingressTlsPort` before it was renamed, to distinguish it from [`OPSCHAIN_INGRESS_TLS_PORT`](#opschain_ingress_tls_port) — a different port, despite the similar name. The old name still works but reports a warning after each deploy; rename it in your `values.yaml`.
:::
...
A hash of options to pass to the vault client, in JSON format. See [`vault.client_options`](/key-concepts/settings.md#vaultclient_options) for the keys OpsChain reads, and the Vault Ruby Client Gem [usage instructions](https://github.com/hashicorp/vault-ruby/tree/master?tab=readme-ov-file#usage) for the rest.
...
- Image registry host, internal host and credentials
- Image pull secret name and the registry credentials it is built from
- Ingress TLS port
...
:::warning[Rotating the Docker Hub credentials]
Apply a change to `OPSCHAIN_DOCKER_USER` or `OPSCHAIN_DOCKER_PASSWORD` as its own upgrade, **before** the one that changes `OPSCHAIN_VERSION`. The runner image copy runs as a pre-upgrade hook, and the image for that hook's own pod is pulled using the image pull secret from the *previous* release, which still holds the old credentials. Rotating and changing the version in one upgrade therefore fails that pull, and the upgrade stops before anything is applied.

The internal image registry credentials have no such constraint. `trow.trow.user` and `trow.trow.password` are read again every time the OpsChain API and its workers start, and they restart on every upgrade, so a change to them takes effect as soon as the upgrade completes.
:::
