...
When a version follows its revision, OpsChain periodically checks the branch or tag for new commits as part of its [background Git fetching](/key-concepts/settings.md#git_remoteperiodic_fetch_interval). When the revision moves, OpsChain resolves the new commit and rebuilds the actions of the assets using that version, without anyone asking for it.
...
Delete an asset template from its actions menu, or delete several at once by selecting them in the templates table and choosing _Delete_ from the _Bulk actions_ menu.
