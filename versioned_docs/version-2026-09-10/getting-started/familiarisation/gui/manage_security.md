...
- `update` - whether the user can create or update the target, for example creating a project
- `execute` - whether the user can execute the target, for example executing a change or a workflow, approving, continuing or rejecting a step, or cancelling a change or workflow run
- `delete` - whether the user can delete the target, for example deleting an environment

:::info[Action precedence]
When any of the `update`, `execute` or `delete` actions are enabled, the `read` action is automatically assigned to the same path.
:::
...
Authorisation rules must be applied to users in order to be effective. An authorisation policy can apply a set of rules to specific users via their username or LDAP group name.
...
#### Who a policy can be assigned to

An assignment names a user OpsChain already has, or one the LDAP directory holds — in which case OpsChain creates the user as the assignment is made, so the policy applies to them straight away rather than from the first time they sign in. A username that is neither is refused, naming the username, rather than being stored against an account that may never exist.

A policy can be assigned to a local user as well as an LDAP one. Where both hold the same username, the policy is assigned to the LDAP user, as that is the account they authenticate as.

Deleting a user removes their policy assignments. Creating an account with the same username later does not restore them.
