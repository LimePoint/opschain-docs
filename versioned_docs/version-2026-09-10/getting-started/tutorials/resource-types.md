...
The actions we've defined in this file have descriptions and therefore will be visible in the OpsChain UI scoped to their respective resource name, `db` and `web` in this case. You can imagine them as lower-level actions that perform actions in the resources individually, but we want to orchestrate them in our `actions.rb` file, providing organized, higher-level actions for any assets created from this template.
:::
...
:::note[Action list]
OpsChain will only show actions that have been given a description in the `actions.rb` files.
:::
...
It might take a few minutes for OpsChain to build the image and process your actions. You can check the status of the action processing in the `Generation history` tab inside the `Actions` section. Each action generation will have its own logs stating any errors or build failures to help you troubleshoot any issues.
:::
...
If we now look at the `Compare versions` section inside the `Properties` tab for this change, we can see that the `last_backup_at` property has been updated to when the backup action was run. The asset's properties are versioned and can be compared between different versions to see what has changed.

![Properties tab](./images/managed-server-action-properties-compare.png)

To make our life simpler, let's schedule the `backup` action to run on a regular basis using a [scheduled activity](/getting-started/familiarisation/gui/scheduled_activities.md). Using OpsChain's UI, we can quickly configure the action to run every day at 10am, for example.

![Scheduled activity dialog](./images/managed-server-scheduled-backup.png)

We've successfully used a single template to perform specialized tasks on different types of assets, with full control over the steps each action takes using the OpsChain properties as the arguments for such actions. With our assets setup, we can now integrate them in a [workflow](/getting-started/familiarisation/gui/workflows.md) to perform multi-environment deployments or updates, with stages, human approval steps and much more.

## What to do next

- Learn how to [integrate secrets in OpsChain](/getting-started/tutorials/secrets.md) to secure your change lifecycle.
- Learn how to [orchestrate changes with workflows](/getting-started/tutorials/workflows.md) to run changes across multiple environments and assets.
