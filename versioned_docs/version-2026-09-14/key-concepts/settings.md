...
These settings let OpsChain automatically grow or shrink the number of worker processes in each pod in response to this queuing, within a configured minimum and maximum, instead of relying solely on a fixed count. Every autoscaling decision is recorded as an [event](/key-concepts/events.md), whether or not the worker count is actually changed.

### api_autoscaler.mode

Default value: _active_<br/>
Accepted values: _disabled_, _dry_run_, _active_<br/>
Scope: _global_

Controls whether autoscaling is allowed to change the worker count:

- `disabled` — autoscaling is turned off.
- `dry_run` — OpsChain evaluates whether it would grow or shrink the worker count and records the decision as an event, but never changes the worker count. Useful for observing autoscaling behaviour before relying on it.
...
:::warning
Each worker process opens its own database connection pool, so a higher `max_workers` also means more total database connections — this is not checked automatically. Make sure your database's [`max_connections`](/advanced/ha/index.md#other-settings) has enough headroom for `max_workers` at full scale, across every OpsChain pod and replica connecting to it, not just this one setting.
:::
...
Whether to record an event for every email OpsChain attempts to send. When enabled, each attempt that reaches your mail server raises either an `info:email:sent` or an `error:email:send_failed` event, giving you an audit trail of the mail OpsChain sent on your behalf and the reason for any of it that did not arrive.

The event records the sender, the recipients, the subject and, for a successful send, the message ID your mail server assigned. A failure records the reason in place of the message ID. The message body is never recorded, and an attachment contributes only its filename and size.

Email sent for a change, an agent or an asset is logged against the node it relates to, so the events appear under that node and the setting can be enabled for a single project or environment at a time. Some email has no related node — a notification sent from a user's own preferences, and the test send from a notification channel — and its events are recorded without one.

:::note
An event with no node is not removed by [data cleanup](/operations/maintenance/data-cleaning.md), which purges the data belonging to the nodes a cleanup definition nominates. Bear this in mind when enabling the setting globally on an instance that sends this kind of email steadily.
:::

:::note
These events never send an email or a Slack message themselves, even when one of your [event subscriptions](/operations/notifications.md#subscribing-to-events) matches them — otherwise sending an email would raise an event that sent another email. Webhook, change and workflow subscriptions do still receive them, within the [loop limits](/operations/notifications.md#notification-loops).
:::

A send that OpsChain rejects before contacting your mail server — one that fails validation, or one attempted while no default global email channel is configured — raises no event of either type. Those failures are reported to the caller, and the missing channel is reported separately as an `api:notification:error` event.

### on_failure.dump_properties

Default value: _false_

When a step fails, OpsChain writes a report to the change logs naming the resource whose action failed, its resource type and its controller class. This setting adds the resolved resource properties to that report.

Set it to `true` to dump the properties of every resource the report covers, or to a list of resource names to dump only those:

```json
{
  "on_failure": {
    "dump_properties": ["prod:database", "prod:web_*"]
  }
}
```

A name is the resource's full path, with each enclosing namespace separated by a colon - the same path the report logs as `Resource: prod:database`. Names are matched as globs, so `prod:web_*` covers every resource under `prod` whose name starts with `web_`, and `*` is equivalent to `true`. Naming the resources you are interested in keeps the logs readable when an `actions.rb` defines hundreds of them.

:::note
The properties of a resource whose controller could not be created are always dumped, whatever this setting says, since the values the controller was given are usually the reason it failed. This applies to a controller that fails while the resource is being defined as well - OpsChain reports the resource that failed by name, which a backtrace into a dynamically generated `actions.rb` cannot.
:::

:::warning
Enabling this will write resolved property values to the change logs. Sensitive values are masked unless [`controller.mask_properties`](/key-concepts/settings.md#controllermask_properties) is set to `false`.
:::
...
When a resource controller is constructed, the key/value pairs supplied to its constructor are checked for possibly sensitive keys and the values added to the logging data masker. Sensitive keys are those that match the regex `/(?:^|_)(?:(?:password|passphrase|passwd)s?\z|(?:secret|token|pass|key)s?(?:_|\z))/i` - `password`, `passphrase` and `passwd` only where the key ends there, so a descriptor such as `password_length` is not treated as a secret, and `secret`, `token`, `pass` and `key` as a whole word anywhere in the key. The same check is applied to a resource's properties before they are written to the change logs by [`on_failure.dump_properties`](/key-concepts/settings.md#on_failuredump_properties), so a resource the step never used is masked even though its controller was never built.

A property held under a sensitive key is resolved if it needs to be: a [lazy](/key-concepts/actions.md#lazy-property-evaluation) value cannot be registered with the masker without being resolved first. Under any other key, a lazy value that has not been resolved yet is left alone rather than resolved so that it can be scanned - so building a controller does not resolve the lazy properties it never reads, and a lazy property that raises only does so if something actually reads it.

A lazy value left alone this way is scanned when it does resolve. A sensitive value nested inside it - `db_config` holding a lazy that resolves to a hash with a `password` in it - is registered with the masker at that point, before whatever asked for the value receives it. Masking follows resolution rather than anticipating it, which is why nothing is resolved early in order to be scanned.

Registering a value is a round trip to the masking process, so each distinct value is sent once per step however many resources carry it - a password held in the project properties and read by five hundred resources is registered once, not five hundred times.

The check walks nested hashes and arrays to whatever depth they reach, and enters each one once, so a structure that refers back to itself - a hash held somewhere beneath one of its own keys - is scanned rather than followed round the loop.

:::warning[A masker that cannot be reached stops the change]
A round trip that fails is not passed over. It is raised, and the step fails: a change that cannot mask its sensitive values stops rather than carrying on and writing them to the logs unmasked.
:::

Setting this to `false` disables all of this masking - the check as the controller is built, the scan of a lazy value as it resolves, and the masking of the properties the dump writes. In conjunction with the `on_failure.dump_properties` setting, this can be useful for debugging controller creation issues by allowing you to see the actual property values in the change logs.
...
- `created_by` - optional object restricting the filter to changes created by particular people. It accepts a `user_names` array of usernames and/or an `ldap_groups` array of LDAP group names. If specified, the change must have been created by one of the listed users, or by a member of one of the listed LDAP groups, to match the filter.
...
:::note[Approver names are validated and matched without regard to case]
On an instance using LDAP authentication, every user and group name you supply — here and in [`created_by`](#change_filter) — is checked against the LDAP directory when the settings are saved. A name that matches no user or group is rejected, so a misspelled approver is reported at that point rather than silently never applying. A name that does match is stored using the directory's own capitalisation.
...
### git_remote.fetch_lock_timeout

Default value: _300_ (5 minutes)<br/>
Accepted values: _1 or greater_<br/>
Scope: _global_

The longest, in seconds, that OpsChain will wait for exclusive access to its copy of a Git remote's repository.

Only one operation works with a given repository at a time. This setting bounds every wait for it, whichever operation is waiting: fetching the remote, building the repository tarball sent to a step runner, and walking a commit range to evaluate a scheduled change's [commit file patterns](/getting-started/familiarisation/gui/scheduled_activities.md#limiting-a-scheduled-change-to-particular-files). An operation that reaches the timeout fails with a message naming the Git remote and the project or environment it belongs to.

Most waits end well within the timeout, because the operation holding the repository is a fetch that takes seconds. The timeout applies when the holder cannot finish — a Git server that has stopped responding, or a network path that has gone away without closing the connection. A waiting operation occupies an OpsChain background worker until it either gets the repository or times out, so this setting also caps how long one unreachable Git remote can tie up capacity that other work needs.

A scheduled change whose commit file patterns hit this timeout is still created and run, as described in [when the patterns cannot be evaluated](/getting-started/familiarisation/gui/scheduled_activities.md#when-the-patterns-cannot-be-evaluated).

Raise this if you have very large repositories whose fetch or tarball takes longer than the default and legitimate work is failing with the timeout. Lower it to give up on an unresponsive Git server sooner. See also [`git_remote.ssh.connect_timeout`](#git_remotesshconnect_timeout) and the two `server_alive` settings, which bound the underlying SSH connection rather than the wait for the repository.
...
Only a fetch that succeeded counts. A caller that was waiting behind a fetch that failed always fetches for itself, whatever the threshold is set to.
...
Default value: _900_ (15 minutes)<br/>
Accepted values: _60 to 86400 (1 day)_<br/>
Scope: _global, project_
...
An individual Git remote can be given an interval of its own, which is used in place of the value resolved here. A remote without one follows its project's interval. See [fetch interval](/getting-started/familiarisation/gui/projects/git_remotes.md#fetch-interval).

When a fetch fails, that remote's interval doubles on each consecutive failure, up to a maximum of one day, and a `warn:git_remote:periodic_fetch:failed` [event](/key-concepts/events.md#git-remotes) is recorded (at most once an hour per remote). The first fetch to succeed again resets the interval and records an `info:git_remote:periodic_fetch:recovered` event. An unreachable remote is therefore retried progressively less often rather than at full rate indefinitely.
...
Default value: _86400_ (1 day)<br/>
Accepted values: _60 or greater_<br/>
Scope: _global_
...
### git_remote.ssh.connect_timeout

Default value: _10_ (seconds)<br/>
Accepted values: _1 or greater_<br/>
Scope: _global, project_

How long, in seconds, SSH will spend establishing a connection to an SSH Git remote before giving up.

This covers the connection only — reaching the server and negotiating the SSH session. It places no limit on an established session, which is what the two `server_alive` settings below do. A Git server that accepts a connection and then stops responding is not caught by this setting.

These settings apply to SSH Git remotes. HTTPS remotes are unaffected. They also reach the step's own SSH Git operations: OpsChain publishes them as [`ssh_settings`](/key-concepts/context.md) in the context, and the [`git_clone` resource](/advanced/resource-types/index.md#checking-out-a-repository-that-is-not-configured-in-the-project) applies them to a checkout from a `url`.

### git_remote.ssh.server_alive_interval

Default value: _30_ (seconds)<br/>
Accepted values: _1 or greater_<br/>
Scope: _global, project_

How often, in seconds, SSH asks the Git server to confirm that an otherwise idle connection is still alive.

Once a fetch is under way SSH waits for the server to respond, with no time limit of its own. If the server or the network path to it disappears without closing the connection — a firewall dropping an idle session, a load balancer restarting, a server that has stopped making progress — nothing tells SSH, and the fetch hangs indefinitely. These keepalives are what detect that case.

A connection is abandoned after `server_alive_interval` multiplied by [`git_remote.ssh.server_alive_count_max`](#git_remotesshserver_alive_count_max) seconds without a response, so 90 seconds with the default values. The fetch then fails as it would for any other unreachable remote, and [`git_remote.periodic_fetch_interval`](#git_remoteperiodic_fetch_interval) backs that remote off rather than retrying it at full rate.

### git_remote.ssh.server_alive_count_max

Default value: _3_<br/>
Accepted values: _1 or greater_<br/>
Scope: _global, project_

How many consecutive unanswered keepalives SSH tolerates before abandoning the connection.

Together with [`git_remote.ssh.server_alive_interval`](#git_remotesshserver_alive_interval) this determines how long a connection to an unresponsive Git server is tolerated. Raising either value makes OpsChain more patient with a slow or intermittently unresponsive server; lowering either detects a dead one sooner, at the cost of abandoning a connection that was only slow.
...
:::warning[Pin the override if the image behind it can change]
OpsChain normally identifies the base runner by its image digest, so a change rebuilds its runner image whenever that digest moves. An override is instead taken at face value as the image you have chosen, and the override string itself identifies the base runner when deciding whether an already-built image can be reused.

That means two changes sharing an override reuse the same built image — which is what you want for a fixed image, but not if the override points at a moving tag, because OpsChain will keep reusing an image built from an earlier version of it. Pin the override to a digest, or to a tag you never move, if each change should pick up a new base runner.
:::
...
An individual action can be exempted from this, reading `actions.rb` again for its own step — see [reloading `actions.rb` for a step](/key-concepts/actions.md#reloading-actionsrb-for-a-step).
...
Default value: _{} (no volumes)_<br/>
Accepted pod types: _`default`, `change_worker`, `step_runner`, `agent`_
...
- `ssl_pem_file`: The path to the SSL PEM file holding the client certificate OpsChain presents to the vault.
- `ssl_pem_passphrase`: The passphrase to use when decrypting the SSL PEM file.
- `ssl_ca_cert`: The path to the certificate authority file to verify the vault's certificate against.
- `ssl_timeout`: The timeout for the SSL connection in seconds.
- `ssl_verify`: Whether to verify the certificate the vault presents. Defaults to `true`. [`vault.ssl_verify`](#vaultssl_verify) takes precedence over this key.
...
You do not normally set the two file paths. OpsChain fills them in when it first configures the bundled secret vault, pointing at the certificate the chart mounts into the OpsChain pods - to serve the vault with your own certificate, set [`secretVault.internalCertificateSecretName`](/setup/configuration/tls/manual-cert-management.md#secret-vault-internal-certificate) and leave these paths as they are. Set them by hand only for an external vault, naming a file you have mounted into the OpsChain pods yourself.

To trust a certificate authority without mounting a file, add it to the [trust store](/setup/configuration/tls/index.md#trusting-additional-certificate-authorities). The authorities the API container trusts are used as well as the one `ssl_ca_cert` names, so an authority in the trust store applies to the vault connection without being named here.
...
This covers every OpsChain connection to the vault, including the ones the API makes as it starts. It takes precedence over the `ssl_verify` key in [`vault.client_options`](#vaultclient_options), so set it here when the two would disagree.
