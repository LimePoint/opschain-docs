...
A running workflow run can be cancelled by anyone with permission to [execute](/getting-started/familiarisation/gui/manage_security.md#authorisation-rule-actions) the workflow — the same permission required to start it. If a workflow run is cancelled, all finalised steps remain in their existing state, and all `pending`, `queued`, or `running` steps are transitioned to the `cancelled` state.
