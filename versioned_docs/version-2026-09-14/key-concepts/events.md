...
- `warning:scheduled_changes:commit_file_patterns` — a scheduled change with [commit file patterns](/getting-started/familiarisation/gui/scheduled_activities.md#limiting-a-scheduled-change-to-particular-files) either could not evaluate them, in which case it created its change anyway, or reached the end of its schedule without them ever being satisfied. Throttled to one event per hour.
...
- `warn:mintmodel:actions_unavailable` — an asset's MintModel actions could not be determined while its actions were being generated, so an `actions.rb` reading them with [`mintmodel_actions`](/key-concepts/actions.md#reading-an-assets-mintmodel-actions) saw an empty catalog. The generation itself continues. Throttled to one event per hour.
...
- `warn:agent:image_missing` — the agent could not be started because its image was not in the registry of the [site](/advanced/ha/index.md) handling the start. A build was started and the agent will automatically start in that site once the build finishes.
...
- `warn:notification_channel:disabled` — a subscription would have been delivered through a notification channel that is disabled, so it was not delivered. Throttled to one event per hour per channel.
- `info:email:sent` — an email was sent, recording the sender, recipients and subject, and the message id the mail server returned. Only recorded while [`log_email_events`](/key-concepts/settings.md#log_email_events) is enabled.
- `error:email:send_failed` — an email could not be sent, recording the sender, recipients and subject, and the reason in the `error` data key. Only recorded while [`log_email_events`](/key-concepts/settings.md#log_email_events) is enabled.
...
#### Sites and database clusters

- `info:database_cluster:registered` — a [site's database cluster](/advanced/ha/index.md#terminology) reported itself for the first time and now appears on the [database screen](/getting-started/familiarisation/gui/database.md).
- `info:database_cluster:role_changed` — a site's database cluster changed between primary and replica, so a failover is visible in the audit history.
