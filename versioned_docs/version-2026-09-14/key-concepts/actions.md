...
- A `default:` applies when the resource does not set the property, or sets it to `nil` - unless the property is declared `keep_nil: true`, described below.
...
- A `coerce:` normalises every value the reader returns, whether the resource supplied it or it came from the `default:`. It is evaluated against the controller, so it can build its result from the other properties, and it is checked where it is declared rather than on the first read: it must be callable and accept the property value as its only argument.
- A coercion is never handed a `nil`, and one that returns `nil` lets the `default:` take over. That is how a controller reads a blank value as an absent one.
- `keep_nil: true` tells the reader that a `nil` the resource set deliberately is a value of its own, so the `default:` applies only to a property the resource never set at all. It is the value the resource set that has to be `nil`, not merely the key that has to be present, and a `nil` a coercion produced is still an absent value.
- `required: true` raises `OpsChain::MissingPropertyError` when the property still reads as missing once the coercion and the default have been applied. Missing means `nil`, or empty - an empty run list is no more use to a controller than no run list at all - so a value that cannot be empty, `false` included, is a value. The message names the property, as `An "environment" property is required`.
- The readers are private, because they are part of your controller's implementation rather than actions on the resource.

Together they let a controller state what it needs, rather than checking for it at every call site:

```ruby
class ChefNodeController < OpsChain::Dsl::BaseController
  property :environment, required: true
  property :run_list, coerce: ->(value) { Array(value).reject(&:empty?) }, required: true
  property :store_in, default: '/chef/result', keep_nil: true
end
```

- `environment` is reported as missing the first time the controller reads it, rather than failing further in with whatever a `nil` environment does next.
- `run_list` accepts a single recipe as a string and reads it as a one element list. A list of blank entries coerces to an empty list, which `required: true` then reports as missing.
- `store_in` writes to `/chef/result` unless the resource says otherwise, and a resource that sets `store_in nil` turns the write back off rather than getting the default.
...
##### Properties shared by a family of controllers

Controllers built around the same tool usually share properties - where the tool's configuration file lives, where the result is written back into OpsChain properties. Declaring them in a module that extends `OpsChain::Mixins::SharedControllerProperties` declares them once, and every controller that includes the module gets them:

```ruby
module ChefProperties
  extend OpsChain::Mixins::SharedControllerProperties

  property :knife_config, default: '/etc/chef/knife.rb'
  property :store_in, keep_nil: true
end

class ChefNodeController < OpsChain::Dsl::BaseController
  include ChefProperties

  property :name, required: true
end
```

`ChefNodeController` accepts `knife_config`, `store_in` and `name`, and reports all three as its resource type properties.

- The properties arrive with every option the module declared them with, the coercions and the `keep_nil` included.
- Each declaration is still validated against the controller that receives it, so a name that collides with something in the including controller is reported against that controller as the class is defined.
- The receiving class only has to be one that takes `property` declarations, so a controller [built from the modules](#adding-the-base-controller-behaviour-to-an-existing-class) rather than from `OpsChain::Dsl::BaseController` accepts a shared properties module too.
- Including the module in a class that does not take `property` declarations at all raises as that class is defined, naming the module it is missing rather than leaving the properties silently undeclared.
...
| `OpsChain::Mixins::SharedControllerProperties` | `extend`, in a module of your own | `property`, declared once and replayed into every controller that includes your module. See [properties shared by a family of controllers](#properties-shared-by-a-family-of-controllers) |
...
A class built this way is a controller in every respect that matters to the modules: it takes `property` declarations with all of their options, contributes actions from the modules it includes, and accepts a shared properties module exactly as a class inheriting from the base controller does.
