...
By default, OpsChain will output the resource type and controller class of the resource whose action has failed during a change. If you would like OpsChain to output all the resources defined in your `actions.rb` when an error occurs, set the `OPSCHAIN_LOG_RESOURCES_ON_ERROR` environment variable to `true` in your change properties.

A controller is only reported as built if the step actually used it. A resource the step never reached is reported as `(not instantiated during this step)` against the controller class it declares - OpsChain does not build a controller in order to describe it, so enabling this on an `actions.rb` that defines hundreds of resources does not construct hundreds of controllers. A resource declared without a controller at all reports `none declared` in place of a class name.

Resource properties are not included unless you ask for them, with the [`on_failure.dump_properties`](/key-concepts/settings.md#on_failuredump_properties) setting - which accepts a list of resource names or globs, so you can see the properties of the resources you are interested in without dumping every resource in the project. The exception is a resource whose controller could not be created: its properties are always dumped alongside the error the constructor raised, since they are usually the reason it failed.

The properties are resolved as they are dumped. A [lazy](/key-concepts/actions.md#lazy-property-evaluation) property is resolved and its value annotated, so a value the property worked out when it was read is distinguishable from one the resource was given outright:

```text
- Resource Properties: {"region" => "eu-west-1", "instance_count" => 3, "connection_settings" => {"endpoint" => "db.example.com"} (LazyPropertyValue resolved)}
```

A property that cannot be resolved is replaced with a description of the failure rather than stopping the dump - the rest of the resource's properties are still reported. A property whose block raised is reported against the file it was defined in, as `<error resolving proc property defined at /path/to/actions.rb: this is broken>`; where the failure came before OpsChain could work out where the property was written it is reported as `<error reading property: ...>`. The angle brackets mark a description of a value rather than the value itself.

### Checking what a `values.yaml` setting actually renders to

The OpsChain Helm chart has several layers - your own `values.yaml`, chart and subchart defaults, and values nested under a subchart's own key (e.g. everything under `openbao:` is passed through to the bundled OpenBao chart) - so it isn't always obvious which layer wins for a given setting, or whether a setting even reached the place you expected. A misspelled or wrongly-nested key is a common cause: Helm does not validate that a key in `values.yaml` corresponds to anything the chart actually reads, so an override placed one level too shallow or too deep is silently ignored rather than raising an error.

`helm template` renders the exact Kubernetes manifests Helm would apply for a given chart version and `values.yaml`, without touching the cluster - making it a safe first step whenever a setting doesn't appear to be taking effect. Run it with the same chart version and `values.yaml` you would use for `helm install`/`helm upgrade`:

```bash
helm template opschain oci://docker.io/limepoint/opschain --version ${OPSCHAIN_CHART_VERSION} -f values.yaml > rendered.yaml
```

Then inspect the rendered manifest for the resource the setting should affect. For example, to confirm how many replicas the secret vault `StatefulSet` will actually be deployed with:

```bash
grep -A2 '^kind: StatefulSet' rendered.yaml | grep -B1 'name: opschain-secret-vault'
grep -A5 'name: opschain-secret-vault$' rendered.yaml | grep replicas
```

If the rendered value isn't what you expected, check the key is nested correctly in your `values.yaml` (following the setting's documented path exactly, including any subchart prefix) before assuming the setting itself doesn't work.

:::tip
`diff` the rendered output of two candidate `values.yaml` files to see the full, precise effect of a single setting change - this is especially useful for a nested or subchart setting, since it shows every resource the key actually affects rather than just the one you assumed:

```bash
diff <(helm template opschain oci://docker.io/limepoint/opschain --version ${OPSCHAIN_CHART_VERSION} -f values-a.yaml) \
     <(helm template opschain oci://docker.io/limepoint/opschain --version ${OPSCHAIN_CHART_VERSION} -f values-b.yaml)
```

:::

:::note
`helm template` shows what a fresh `install`/`upgrade` **would** render - it does not tell you what is currently running in the cluster. To check what the currently installed release was actually deployed with, use `helm get values -a opschain -n ${OPSCHAIN_KUBERNETES_NAMESPACE}` (the full, merged values Helm recorded for that release) or inspect the live resource directly, e.g. `kubectl get statefulset opschain-secret-vault -n ${OPSCHAIN_KUBERNETES_NAMESPACE} -o jsonpath='{.spec.replicas}'`. A difference between the rendered manifest and the live resource usually means the corresponding `helm upgrade` hasn't been run yet.
:::
...
### Pod health checks fail with `no route to host`

Shortly after installing cert-manager, the CNPG operator, or OpsChain itself, pods stay unready and their events show liveness or readiness probes failing to reach a pod IP (an address in the `10.42.0.0/16` range) with `no route to host`:

```text
Warning  Unhealthy  pod/cert-manager-webhook-...  Liveness probe failed: Get "http://10.42.0.9:6080/livez": dial tcp 10.42.0.9:6080: connect: no route to host
```

`10.42.0.0/16` is the K3s pod network. The `no route to host` result — rather than a connection timeout — is an ICMP `host-prohibited` reject, which is the signature of `firewalld` rejecting the packet in its default zone. In other words the host firewall is blocking the kubelet's probe to the pod's own IP. This happens when the firewall is enabled but the K3s pod and service networks have not been added to its trusted zone.

#### Solution - trust the K3s pod and service networks

The [firewall setup](/setup/installing_k3s.md#firewall-setup) step adds the pod (`10.42.0.0/16`) and service (`10.43.0.0/16`) networks to `firewalld`'s trusted zone, so the firewall only restricts external traffic and not in-cluster networking. Pods failing their probes with `no route to host` almost always means that step was missed. Add the sources and reload:
...
The failing pods recover on their next probe. If they do not, restart K3s so it re-applies its own firewall rules on top of the reloaded ruleset:
...
:::note
These CIDRs are the K3s defaults. If you overrode the cluster or service CIDR when installing K3s, trust your own ranges instead — `kubectl cluster-info dump | grep -m2 cluster-cidr` shows the values in use.
:::

#### Solution - disable the host firewall

`firewalld` is known to conflict with K3s. If you do not need a host firewall on this node, disabling it avoids the issue entirely:

```bash
sudo systemctl disable --now firewalld
```

### Secret vault requests fail with `no route to host`

A step fails while OpsChain is reading a secret, reporting that it could not open a connection to the secret vault's *external* hostname:

```text
Failed to open TCP connection to vault.my-company.com:17001 (No route to host - connect(2) for "vault.my-company.com" port 17001) (Errno::EHOSTUNREACH)
    from .../gems/vault-0.18.2/lib/vault/persistent.rb:693:in 'Vault::PersistentHTTP#start'
```

OpsChain connects to the secret vault inside the cluster, so an external hostname appearing here means the request was redirected. The secret vault runs as several replicas, and a request that lands on a standby replica is answered with a redirect to the active one at the address the vault advertises - [`global.secretVaultExternalHostName`](/setup/configuration/tls/index.md#secret-vault-hostname) and [`global.secretVaultExternalPort`](/setup/configuration/additional-settings.md#globalsecretvaultexternalport). The client follows that redirect, so it fails whenever the advertised address is not one the client can reach.

Two properties of this failure are worth knowing before diagnosing it, and they apply equally to [`Name or service not known`](#secret-vault-requests-fail-with-name-or-service-not-known) below:

- **It is intermittent.** Only the requests that happen to land on a standby replica are redirected, so the same action can succeed and fail in turn.
- **A single-replica secret vault never shows it,** because there is no standby to redirect.

Which of the two errors you get says where the redirect broke. `no route to host` means the hostname resolved but nothing answered on that port, so read on. `Name or service not known` means the hostname did not resolve at all - see [the next entry](#secret-vault-requests-fail-with-name-or-service-not-known) instead.

#### Diagnosis - check the address the vault advertises

Ask the vault which address it hands out, rather than reading the deployed configuration back:

```bash
kubectl -n ${KUBERNETES_NAMESPACE} exec deploy/opschain-api -- curl -sk https://opschain-secret-vault:8200/v1/sys/leader
```

The `leader_address` it reports is exactly what a standby redirects callers to. Compare its **port** against the port clients use to reach OpsChain from outside the cluster. They differ when a load balancer in front of the cluster translates the port - if it accepts connections on 443 and forwards to the node on 17001, clients dial 443, and an advertised port of 17001 is unreachable for them.

#### Solution - advertise the port clients dial

Set the externally dialled port in your `values.yaml` and redeploy:
...
Leave it unset if nothing translates the port; it then follows the port Kong's proxy service publishes, which is correct in that case. Note this is **not** [`OPSCHAIN_INGRESS_TLS_PORT`](/setup/configuration/additional-settings.md#opschain_ingress_tls_port), which is the port the cluster itself uses to reach Kong and should stay as it is.

#### Solution - recycle the secret vault replicas

Each replica reads the advertised address into its environment when it starts, so a deploy that changes the address leaves the running replicas advertising the old one. Recycle them - standby replicas first, and the active one last, so that the replica which takes over leadership is already running the new value:

```bash
kubectl -n ${KUBERNETES_NAMESPACE} delete pod -l 'app.kubernetes.io/name=openbao,component=server,openbao-active=false'
# wait for those replicas to become ready again, then release the active one
kubectl -n ${KUBERNETES_NAMESPACE} delete pod -l 'app.kubernetes.io/name=openbao,component=server,openbao-active=true'
```

:::note[`kubectl rollout restart` does not work here]

The secret vault's StatefulSet uses the `OnDelete` update strategy, which `kubectl` refuses to restart. Deleting the pods, as above, is the supported way to recycle them.

:::

Re-run the diagnosis command afterwards - `leader_address` should now report the port your clients dial.

### Secret vault requests fail with `Name or service not known`

A step fails while reading a secret, or the OpsChain API fails to start after an upgrade, reporting that the secret vault's *external* hostname could not be resolved:

```text
Failed while checking if vault is configured: {"errors":[{"rest_exception":"Failed to open TCP connection to vault.my-company.com:443 (getaddrinfo: Name or service not known)"}]}
```

This is the same standby redirect described in [secret vault requests fail with `no route to host`](#secret-vault-requests-fail-with-no-route-to-host), including its intermittency, failing one step earlier: the caller cannot resolve the advertised hostname at all, rather than failing to connect to it. The hostname is often configured only in the DNS your client machines use, so that it reaches the secret vault UI from a browser, while the pods inside the cluster cannot resolve it.

Because the API contacts the vault as it starts, this can present as an upgrade that never completes rather than as a failing change - see [`helm upgrade` fails with `context deadline exceeded`](#helm-upgrade-fails-with-context-deadline-exceeded).

#### Diagnosis - resolve the hostname from inside the cluster

```bash
kubectl -n ${KUBERNETES_NAMESPACE} exec -it deploy/opschain-api-worker -- getent hosts vault.my-company.com
```

No output means the pods cannot resolve it, whatever your client machines can do.

#### Solution - make the hostname resolve inside the cluster

Add it to your organisation's DNS, or to CoreDNS with a `coredns-custom` ConfigMap. The [secret vault hostname](/setup/configuration/tls/index.md#secret-vault-hostname) section covers both, including the ConfigMap to apply and the high availability case, where every cluster's CoreDNS must resolve every other cluster's secret vault hostname.

No pod recycling is needed - name resolution is not read into the vault's environment at start up, unlike the advertised address itself.

### Secret vault requests fail with `certificate verify failed`

The OpsChain API fails to start, or a step fails while reading a secret, reporting that the secret vault's certificate could not be verified:

```text
Failed while checking if vault is configured: SSL_connect returned=1 errno=0 peeraddr=10.42.0.28:8200 state=error: certificate verify failed (unable to get local issuer certificate)
```

The secret vault runs as several replicas, and a request that lands on a standby is redirected to the active replica at the secret vault's external hostname, which the ingress serves with its own certificate. That certificate does not have to come from the same authority as the vault's internal certificate, and OpsChain has to trust both - see [trusting additional certificate authorities](/setup/configuration/tls/index.md#trusting-additional-certificate-authorities).

Because the API contacts the vault as it starts, this usually presents as an API pod that never becomes ready, rather than as a failing change - see [`helm upgrade` fails with `context deadline exceeded`](#helm-upgrade-fails-with-context-deadline-exceeded).

#### Diagnosis - identify the authority OpsChain does not trust

Print the issuer of the certificate served at the secret vault's external hostname, using your own hostname and [ingress TLS port](/setup/configuration/additional-settings.md#opschain_ingress_tls_port):

```bash
kubectl -n ${KUBERNETES_NAMESPACE} exec deploy/opschain-api-worker -- bash -c 'openssl s_client -connect vault.my-company.com:3443 -servername vault.my-company.com </dev/null 2>/dev/null | openssl x509 -noout -issuer -subject'
```

Compare the issuer it reports against the authorities OpsChain trusts. The ones you have added are listed in the **CA certificates** section of the system configuration page; the rest are the public authorities the container image trusts by default. If the issuer is neither, this is the cause.

#### Solution - trust the authority

Add the authority to `trustStore.certificates` in your `values.yaml` file and upgrade:
...
Helm writes the certificate before the API pod is created, so this works while the API is down. See [trusting additional certificate authorities](/setup/configuration/tls/index.md#trusting-additional-certificate-authorities) for the other ways to supply one.

#### Solution - start the API with verification relaxed

Where the authority's certificate is not to hand, you can start the API without verifying the vault's certificate. Set [`vault.ssl_verify`](/key-concepts/settings.md#vaultssl_verify) to `false` in your `values.yaml` and upgrade:

```yaml
env:
  OPSCHAIN_VAULT_SSL_VERIFY: "false"
```

That is enough to get the instance running while you obtain the certificate. Remove the setting and upgrade again as soon as you have - every secret OpsChain reads and writes travels over that connection.

### A secret stored through the API cannot be resolved by a change

A secret vault path containing a hyphen reached the vault percent-encoded, so a secret stored through the API or the GUI at `/projects/MP-001/secrets/db_password` was written to `MP%2D001`, while action code resolving that same path looked under `MP-001` and did not find it.

New secrets are stored at the decoded path. Secrets stored before upgrading stay where they were written, so move them with the `opschain:migrate_vault_secret_paths` rake task. Run it from your OpsChain server configuration directory (e.g. `~/opschain-configuration`):

```bash
source .env
kubectl exec -ti -n ${OPSCHAIN_KUBERNETES_NAMESPACE} deploy/opschain-api-worker -- /usr/bin/container_start.sh 'rake opschain:migrate_vault_secret_paths'
```

The task reports every encoded path it found alongside the decoded path it would move to, and the keys each holds, without changing anything. Pass `true` to move them:

```bash
kubectl exec -ti -n ${OPSCHAIN_KUBERNETES_NAMESPACE} deploy/opschain-api-worker -- /usr/bin/container_start.sh 'rake opschain:migrate_vault_secret_paths[true]'
```

A path is reported as `SKIP (target exists)` and left alone where the decoded path already holds a secret. Compare the two, remove the one you do not want, and run the task again.

Where a project, environment or asset stores its secrets in a vault of its own, pass its node path as a second argument so the task connects to that vault rather than the global one:

```bash
kubectl exec -ti -n ${OPSCHAIN_KUBERNETES_NAMESPACE} deploy/opschain-api-worker -- /usr/bin/container_start.sh 'rake opschain:migrate_vault_secret_paths[true,/projects/MP-001]'
```
