...
OpsChain will automatically restart the log aggregator deployment if it's stuck or unresponsive.

:::warning
While Fluentd is down, no matter the reason, OpsChain will not start any agents, builds, changes or workflow runs.
:::

If Fluentd never recovers, you can check its logs and events to investigate the issue.
...
The log aggregator picks the change up on its own, without restarting: it watches its configuration and asks Fluentd to reload, so log forwarding continues uninterrupted while the new outputs take effect. When in a [high availability deployment](/advanced/ha/index.md), the configuration is applied to every site.
