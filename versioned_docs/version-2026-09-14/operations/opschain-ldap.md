...
Nesting is not a cause of either. OpsChain searches the whole subtree beneath the configured `user_base` and `group_base`, so users and groups held in sub-organisational units below the base are found in the same way as those directly beneath it. A base that does not contain the entries at all is still too narrow.
