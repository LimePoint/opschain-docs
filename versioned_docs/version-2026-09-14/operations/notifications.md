...
You can configure as many channels as necessary. However, only one enabled channel can be marked as the default channel for each notification type.
...
A disabled channel is never used, even while it remains marked as the default. If the only default channel for a type is disabled, OpsChain behaves as though no default has been configured — notifications of that type are not sent, and an event is recorded telling you the default channel is missing. A disabled default does not stand in the way of nominating a replacement, so you can mark another channel as the default without first clearing the disabled one.
:::
...
The practical consequence is that being able to commit action code to a project's repository, and to run changes in it, carries the ability to send email through this server as your organisation. Where that matters, enforce your sender policy at the SMTP server OpsChain relays through rather than relying on action code to be well behaved — for example by restricting the sender addresses the OpsChain credentials are permitted to use, and by publishing SPF and DMARC records for the domains involved.

Only the short-lived API key OpsChain issues to a running change or agent can send email this way — a personal API key cannot. Every message is recorded as an `api:email:create` [event](/key-concepts/events.md), attributed to the user who started the change or agent and capturing the sender, the recipients, the subject and each attachment's filename and size, so you can review what has been sent. The message body and the attachment content are never recorded.

Action code uses only the connection settings of this channel. The _from_ address you configure here applies to notifications — email sent from action code uses whichever sender address the action supplies, or OpsChain's no-reply address when it supplies none.
:::

:::note
Email notifications are delivered as a formatted HTML table showing the details of the event (such as the project, environment or asset hierarchy, the action or workflow, the initiator and relevant links). Notifications sent to Slack use a more concise text format.
:::
...
:::note
A default email channel must be configured and enabled in the administration notification settings before you can receive email notifications.
:::
...
:::note
A default Slack channel must be configured and enabled in the administration notification settings before you can receive Slack notifications.
:::
...
A subscription can start a change or a workflow run, and that change or workflow run raises events of its own. Where those events match another subscription, the two feed each other and the chain never ends.

OpsChain counts the subscriptions between an event and whatever a person started. A change or workflow run started by a subscription inherits that count and adds one, and the events it raises carry the result. Once more than ten subscriptions separate an event from its origin, it is no longer delivered to any subscription. A change you run yourself starts the count at zero, so your own changes and the notifications they send are unaffected.

The events reporting a notification OpsChain could not deliver — `api:notification:error`, `warn:notification_channel:disabled`, `api:event_filter:error` and `api:event_subscriber:error` — are held to a shorter limit, as are the events [`log_email_events`](/key-concepts/settings.md#log_email_events) records. They are never delivered to a subscription that notifies a user, sends an email or sends a direct message, because delivering one would raise another. Webhook, change and workflow subscriptions do receive them, but only one hop from the change, workflow run or request a person started: an email sent by a change that a subscription itself started is recorded and delivered nowhere.

An event OpsChain does not deliver is still recorded, and appears in the [events](/key-concepts/events.md) list like any other.
