...
| **Archived**    | Indicates the archival status of the environment.       |
...
| **Bulk actions**       | Perform operations on multiple environments, such as archiving, restoring or permanently deleting them. |
...
Archive an environment by selecting it from the environments table and choosing _Archive_ from the _Bulk actions_ menu.
...
Delete an environment from its actions menu, or delete several at once by selecting them in the environments table and choosing _Delete environments_ from the _Bulk actions_ menu.

:::warning
Deleting an environment is permanent and irreversible, and removes the environment along with the assets it owns. An environment that has changes recorded against it cannot be deleted - [archive](#archiving-an-environment) it instead.
:::
