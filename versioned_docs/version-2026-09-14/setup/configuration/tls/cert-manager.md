...
Whichever CA is used, its certificate needs to be trusted in two places: by containerd on each K3s node, so that Kubernetes can pull runner and agent images from the OpsChain image registry, and by any system you use to reach the OpsChain API, UI and secret vault (if you use `cert-manager` to manage those certificates too). Both are done after installing OpsChain — see [setup the custom CA](/setup/setup-instance.md#setup-the-custom-ca) for the K3s nodes, and [trusting the CA certificate](#trusting-the-ca-certificate---post-installation) for everything else.
...
Client machines each have their own way of trusting a certificate, so follow the documentation for yours. On the RHEL, OEL or AlmaLinux VM hosting OpsChain, add it to the system trust store:

```bash
sudo cp opschain-ca.pem /etc/pki/ca-trust/source/anchors/opschain-ca.pem
sudo update-ca-trust
```

That covers tools running on the host, such as `curl` and the OpsChain CLI. Trusting the certificate for containerd, so that K3s itself can pull images from the OpsChain image registry, is a separate step - see [setup the custom CA](/setup/setup-instance.md#setup-the-custom-ca).
...
Once it shows `READY  True`, re-extract and re-trust the CA certificate as described in [trusting the CA certificate](#trusting-the-ca-certificate---post-installation) above, then re-run the [K3s registry trust](/setup/setup-instance.md#setup-the-custom-ca) steps so containerd picks up the change too.
