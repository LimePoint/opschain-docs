...
`migration_ops_model_processor` is an OpsChain resource for running targeted actions against a MintPress environment — applying patches, running transfers or executes, and controlling server/system component lifecycle — without needing to author a full Bamboo plan.
...
Once declared, the resource is driven purely by which action you invoke and what you set in your OpsChain properties for that run:
...
Applies (or rolls back, depending on the patch's own configuration) a single patch, identified by `patch_id`, against `installation` — regardless of which lifecycle stage it's normally associated with.
...
Applies (or rolls back) a set of patches together as one combined OPatch operation (`napply`/`nrollback`), identified by `patch_ids`, against `installation`. All patches in the list must resolve to the same action (all-apply or all-rollback); mixed sets are rejected.

Because it relies on OPatch's combined `napply`/`nrollback`, every patch in the list must have a plain numeric ARU and must not have `execute_before`/`execute_after`/`custom_opatch_command` configured — any patch that does isn't eligible for this combined operation and needs to be applied individually via `apply_patch` instead.
...
Applies (or rolls back, depending on each patch's own configuration) every patch defined for `installation`, in one run — no need to name individual patches. Patches configured to roll back are run first, in the reverse order of their definition and  then the apply patches are run, in the order they're defined.
