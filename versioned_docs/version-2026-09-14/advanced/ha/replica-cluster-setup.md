...
An OpsChain **site** is one complete deployment: a Kubernetes cluster running an API, its workers, and its own **database cluster**. A high availability topology is several sites, kept in sync by replicating one site's database cluster to the others.
