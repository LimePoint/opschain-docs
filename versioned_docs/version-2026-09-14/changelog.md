...
### Enhancements {/* #2026-09-14-enhancements */}

- The [`on_failure.dump_properties`](/key-concepts/settings.md#on_failuredump_properties) setting now accepts a list of resource names as well as `true` or `false`, so a failed step can dump the properties of just the resources you name.
- A failed step's resource report no longer builds the controllers of the resources the step did not use. See [output resource attributes on error](/troubleshooting.md#output-resource-attributes-on-error).
- A controller property can now normalise the value its reader returns, with `coerce:`. See [controller properties](/key-concepts/actions.md#controller-properties).
- A controller property can now be declared `required: true`. See [controller properties](/key-concepts/actions.md#controller-properties).
- A controller property can now keep a `nil` the resource set deliberately, with `keep_nil: true`. See [controller properties](/key-concepts/actions.md#controller-properties).
- Properties shared by a family of controllers can now be declared once, in a module. See [properties shared by a family of controllers](/key-concepts/actions.md#properties-shared-by-a-family-of-controllers).
- Changes and workflow runs are now listed most recently created first, so the latest activity leads every list rather than being truncated away by the list limit.
- The `terraform_config` resource type now offers the `test` action and the `cloud_run`, `filter`, `filters`, `test_directory` and `verbose` properties, following the move to RubyTerraform 1.9.0. See [`opschain-terraform`](/advanced/resource-types/index.md#opschain-terraform).

### Bug fixes {/* #2026-09-14-bug-fixes */}

- A failed step's resource report no longer replaces the failure it was reporting when the failed action's name matches a resource type.
- Building a resource controller no longer resolves every [lazy](/key-concepts/actions.md#lazy-property-evaluation) property on the resource, only those held under a key the masking check treats as sensitive. See [`controller.mask_properties`](/key-concepts/settings.md#controllermask_properties).
- A sensitive value held inside a lazy property is now registered with the log masker when that property resolves, so it is masked even though nothing looked inside the lazy value when the controller was built. See [`controller.mask_properties`](/key-concepts/settings.md#controllermask_properties).
- A resource declared without a controller is now reported as `none declared` rather than as `NilClass`. See [output resource attributes on error](/troubleshooting.md#output-resource-attributes-on-error).
- A resource whose controller cannot be created now reports the properties the constructor was given, and no longer runs the constructor a second time to produce the error message. See [`on_failure.dump_properties`](/key-concepts/settings.md#on_failuredump_properties).
- A controller that fails while its resource is being defined now names the resource it belongs to. See [`on_failure.dump_properties`](/key-concepts/settings.md#on_failuredump_properties).
- Masking a property structure that refers back to itself no longer fails the step with a stack overflow. See [`controller.mask_properties`](/key-concepts/settings.md#controllermask_properties).
- A [lazy](/key-concepts/actions.md#lazy-property-evaluation) property is now annotated as lazy in a failed step's resource report whatever it resolved to, rather than only when its value ended in a `>` - which corrupted the rendering of a hash. See [output resource attributes on error](/troubleshooting.md#output-resource-attributes-on-error).
- A property a failed step's resource report could not read is now reported inside a matching pair of angle brackets. See [output resource attributes on error](/troubleshooting.md#output-resource-attributes-on-error).
- Loading the OpsChain DSL no longer turns Ruby's warnings off for the rest of the process, so a runner started with warnings enabled keeps them.
- The `on_failure` and `controller` settings are now validated, so a misspelled key or a wrongly typed value is reported when the settings are saved rather than ignored at run time.
- Fixed an issue where comparing two nodes reported each node's oldest change as its latest change.
- The changes endpoints no longer advertise a `filter[waiting_for_approval_by]` named filter. It was listed in the API reference but returned an error rather than filtering anything. See [named filters](/advanced/api-filtering.md#named-filters).

## [2026-09-10]

### Before upgrading {/* #2026-09-10-before-upgrading */}

:::warning[The API workers are stopped while the database is migrated]

`helm upgrade` scales `opschain-api-worker` to zero and waits for the work its pods are running to finish, before the new API applies the migrations and the workers return. Schedule downtime for this upgrade to ensure no changes or workflow runs are in flight.

:::

:::warning[Your LDAP directory must be reachable while you upgrade]

This release records the user against a change, an authorisation policy assignment and every other record that names one as a reference to their account rather than as a name, resolving the accounts it does not already hold from the directory.

An upgrade that cannot resolve the last remaining assignee of a system policy — the superuser policy, for example — stops rather than removing that access, and can be run again once the directory is reachable.

:::

### Added {/* #2026-09-10-added */}

- OpsChain can now record an event for every email it attempts to send, giving you an audit trail of the mail sent on your behalf and the reason for any of it that did not arrive. A successful send raises an `info:email:sent` event and a failure raises an `error:email:send_failed` event, each recording the sender, recipients and subject. Set the new [`log_email_events`](/key-concepts/settings.md#log_email_events) setting to `true` to enable it, globally or for a single project, environment or asset.
- Certificate authorities can now be trusted from your `values.yaml` file, using the new `trustStore.certificates` value. Helm writes these before the API pod is created, so they are trusted the first time it starts — which uploading one in the system configuration page cannot do when the authority is the one the API needs in order to start. Certificates supplied this way sit alongside the uploaded ones, which an upgrade leaves untouched. See [trusting additional certificate authorities](/setup/configuration/tls/index.md#trusting-additional-certificate-authorities).
- Resource controllers can now be built on a base class that supplies everything a controller needs, so a controller carries only the code that does its work. `OpsChain::Dsl::BaseController` derives the controller's actions and properties from the class itself, and the same behaviour is available as modules to a controller that already has a superclass of its own. Controllers written without the base class keep working unchanged. See [controller](/key-concepts/actions.md#controller).
- An action can now be set to read its project's `actions.rb` again before it runs, so it sees the state as it is when the step starts rather than as it was when the change began — a property an earlier step set, for example. See [reloading `actions.rb` for a step](/key-concepts/actions.md#reloading-actionsrb-for-a-step).
- The events endpoint can now be filtered by a workflow run, returning every event for the run and its own workflow steps with `filter[for_workflow_run]`, or those of the changes the run spawned as well with `filter[for_workflow_run_nested]`. See [named filters](/advanced/api-filtering.md#named-filters) and the [API reference](pathname:///api-docs/#tag/Events).
- Steps can now be [skipped](/key-concepts/changes.md#skipping-steps) when a change is started, rather than only when one is retried. **Mark steps to skip** in the run change dialog ticks them off the chosen action's step tree, and the selection applies to a scheduled change and to a rerun. See [marking steps to skip](/getting-started/familiarisation/gui/activity.md#marking-steps-to-skip).
- An input step's arguments can now be answered when the change is started, so a change need not stop to ask for values the person starting it already knows. Answer every required argument, tick **Automatically continue wait steps**, and the change runs through its input steps without pausing. See [answering input arguments when starting a change](/key-concepts/actions.md#answering-input-arguments-when-starting-a-change).
- The environment variables a change or workflow run's steps run with can now be set from a table of names and values, rather than by hand-writing nested JSON in the property overrides. See [setting environment variables](/getting-started/familiarisation/gui/activity.md#setting-environment-variables).
- A scheduled change that triggers on new commits can now be held back unless those commits touched particular files, so a schedule that deploys can ignore a commit that only edited documentation. See [limiting a scheduled change to particular files](/getting-started/familiarisation/gui/scheduled_activities.md#limiting-a-scheduled-change-to-particular-files).
- A cleanup job can now remove the finished background tasks a newer task of the same kind has replaced, along with their logs. Nothing else removed these, so an instance that had been running for a while held every action generation and MintModel generation it had ever performed. See [what a cleanup job can remove](/getting-started/familiarisation/gui/data_cleanup.md#what-a-cleanup-job-can-remove).
- A cleanup job can now be set to remove job history from the GUI, as **Delete job history**. The option was previously only reachable over the API. See [what a cleanup job can remove](/getting-started/familiarisation/gui/data_cleanup.md#what-a-cleanup-job-can-remove).
- The site an agent runs at is now shown, and an agent running at another site is listed as running rather than as a fault. Its own logs are readable from any site; its debug logs are only available at the site holding it, which the debug log view now says. See [where an agent runs](/getting-started/familiarisation/gui/projects/agents.md#where-an-agent-runs).
- The revision on the activities table now opens the detail behind the truncated label — the Git remote, the revision, the full commit SHA and a link to the commit — and links a templated change to its template version. See [activity details](/getting-started/familiarisation/gui/activity.md#activity-details).
- A `date` input argument is now answered with a date picker rather than a text field, so the format cannot be typed wrongly. See [input steps](/key-concepts/actions.md#input-steps).
- The jobs list in system information now shows which queue each job is on, and can be filtered to a selection of queues. Each queue now also reports what the job that runs on it does.
- A node's actions are now available from a new actions endpoint, which serves the action catalog and an individual action's step tree. See the [API reference](pathname:///api-docs/#tag/Actions).
- The administration screen's **Database** tab now shows every site in a [high availability topology](/advanced/ha/index.md). Each site reports which one holds the data the others copy from, how far behind the copies are, whether its workers are picking work up and when its API last started. A site appears on its own once anything in it reports in.
- A change now shows the site it ran at, on the change's details and in the API. Every step of a change runs in the same site. See the [high availability](/advanced/ha/index.md#where-work-runs) guide for more information.
- Unused or unavailable agent images are now cleaned up by the [registry reconciliation](/operations/maintenance/container-image-cleanup.md#registry-reconciliation) CronJob.
- The MintModel on an asset's **Latest** tab can now be downloaded as a JSON file. See [asset templates with a MintModel](/getting-started/familiarisation/gui/projects/asset_templates.md#asset-templates-with-a-mintmodel).
- An asset's `actions.rb` can now read the actions its MintModel contributes, with the new `mintmodel_actions` keyword, so a template can build its step tree from what the MintModel contains rather than naming a MintModel path and leaving OpsChain to graft the subtree in. See [reading an asset's MintModel actions](/key-concepts/actions.md#reading-an-assets-mintmodel-actions).
- Chef Infra Server objects can now be managed from action code, with the new `chef_environment` and `chef_node` resource types. Between them they set what the next `chef-client` run does: which cookbook versions an environment resolves, and the run list each node carries. See [OpsChain Chef](/advanced/resource-types/index.md#opschain-chef).

### Changed {/* #2026-09-10-changed */}

- A chain of changes and workflow runs started by [event subscriptions](/operations/notifications.md#notification-loops) now ends. OpsChain counts the subscriptions between an event and whatever a person started, and stops delivering an event once more than ten separate the two, so a subscription that starts a change on an event that change itself raises no longer runs indefinitely. A change you run yourself starts the count at zero and is unaffected.
- The run change and run workflow dialogs now show everything at once in two columns rather than splitting it across tabs — what the change targets on the left, the optional panels on the right. See [running an activity](/getting-started/familiarisation/gui/activity.md#running-an-activity).
- Every step tree — a change's, a workflow run's, the workflow editor's preview and an asset's action tree — is now drawn by one viewer, which handles a large tree without the delay that used to come with one. It adds a minimap, an **Expand / collapse** control, a search that covers step descriptions, and the tree's total step count. See [navigating the step tree view](/getting-started/familiarisation/gui/activity_details.md#navigating-the-step-tree-view).
- Docker Hub and internal image registry credentials are now owned by your `values.yaml` file and can no longer be changed through the GUI or API, so rotating the internal registry's password in a deploy reaches the stored credential. Set them in your `values.yaml` and redeploy to change them — in a [high availability deployment](/advanced/ha/index.md), at every site. The stored copies are removed when you upgrade. See [settings managed by the deployment](/setup/configuration/additional-settings.md#settings-managed-by-the-deployment).
- A failed MintModel generation's render logs are now read when you open them, rather than travelling with every poll of the asset.
- The LDAP directory refresh button now says that it requires a superuser when it is disabled. See [OpsChain LDAP](/operations/opschain-ldap.md).
- The list of actions on an asset's **Actions** tab can now be resized against the step tree beside it, and a long action name is no longer trimmed to fit. See [the asset's action tree](/getting-started/familiarisation/gui/projects/assets.md#the-assets-action-tree).
- Starting an agent now checks that its image is really there before creating the container and builds it and starts the agent if it is not, recording a `warn:agent:image_missing` event so the wait is visible rather than silent. See [agent images](/getting-started/familiarisation/gui/projects/agents.md#agent-images).
- A change to your [log forwarding configuration](/operations/log-forwarding.md) is now applied to every site in a high availability deployment, rather than only the site that served the request. A site added to the deployment later applies the stored configuration when its OpsChain API starts. The log aggregator now picks a change up on its own as well, so there is no longer anything to restart: it watches its configuration and reloads Fluentd in place, and log forwarding is uninterrupted while it does.
- A repeating schedule is now set by picking a common one — every 15 minutes, every weekday at 9:00 am — or by building it from a frequency and a time, with a text box for an expression the builder cannot represent. This applies wherever OpsChain asks for a cron schedule: the run dialogs, a scheduled activity and a cleanup job. See [setting a schedule](/getting-started/familiarisation/gui/scheduled_activities.md#setting-a-schedule).
- A scheduled activity can now be changed in place. Its edit form runs the same panels as the run dialogs, so everything the schedule carries can be edited — previously only its schedule, comments, **Run once** and **New commits only** could, and anything else meant recreating the schedule. What the schedule runs stays fixed. See [editing a scheduled activity](/getting-started/familiarisation/gui/scheduled_activities.md#editing-a-scheduled-activity).
- The steps and workflow steps endpoints now report a truncated response the way the other list endpoints do, with a `206 Partial Content` status code and `partial_response` in the response's `meta`. Both still return 1000 steps and cannot be asked for more. See [response limits](/advanced/api-filtering.md#response-limits).
- A failed Git remote fetch now records what git reported. The output is attached to the event raised for the failure and, where a change triggered the fetch, written to the change's log, with any credentials in the fetch command masked.
- A template version's Git revision can now be refreshed while a change is running against it. A change resolves its own commit when it is created and keeps running against that commit, so the refresh no longer reaches it. A version currently assigned to an asset or agent still refuses the refresh unless it is forced.
- `git_clone` is now available in `actions.rb` without requiring `opschain-git-clone` first. See [the `git_clone` resource](/advanced/resource-types/index.md#opschain-git-clone).
- Deleting a user now removes their authorisation policy assignments, so recreating an account of the same name does not restore its access.
- An asset's generation history now loads a page at a time. It returned every task ever recorded against the asset's template version, which nothing purges, so the list grew for the asset's lifetime. See [response limits](/advanced/api-filtering.md#response-limits).
- Work waiting on one of the `concurrent` capacity pools now starts as soon as a slot frees up, rather than waiting for a worker's next poll. See [`concurrent.runner_limit`](/key-concepts/settings.md#concurrentrunner_limit).
- A step's pod is now watched rather than polled while it starts, so a transition is reported as it happens and a waiting step makes far fewer requests of Kubernetes.
- Improved the performance of listing changes, which ran an extra query for every running change on the page.
- Improved the performance of listing events, which resolved each event's source, node and subscriber one row at a time.
- The secret vault container now asks for CPU and memory of its own, so the node no longer treats it as the first workload to starve or evict under pressure.
- The [API reference](pathname:///api-docs/) now documents four endpoints it had omitted and corrects the request and response shapes it described wrongly, among them the authentication token responses, the workflow run create request and the workflow run step tree.

### Removed {/* #2026-09-10-removed */}

- The node response no longer includes the node's actions. Read them from the [actions endpoint](pathname:///api-docs/#tag/Actions) instead.
- The GitHub credential settings have been removed. They were offered by the settings editor, refused when saved, and read by nothing.
- A scheduled change or workflow run can no longer be moved to a different project, environment or asset. `parent_path`, `project_code`, `environment_code` and `asset_code` are rejected when updating a scheduled activity - create a new scheduled activity against the node you want instead. The move was never checked against the destination node's permissions, and one that collided with an existing schedule failed with a server error. See [editing a scheduled activity](/getting-started/familiarisation/gui/scheduled_activities.md#editing-a-scheduled-activity).

### Fixed {/* #2026-09-10-fixed */}

- The OpsChain API now verifies the secret vault's certificate against the certificate authorities its container trusts, as well as the one named in [`vault.client_options`](/key-concepts/settings.md#vaultclient_options). Only the configured authority was used, so an installation serving the secret vault's external hostname with a certificate from a different authority could not start the API at all. See [secret vault requests fail with `certificate verify failed`](/troubleshooting.md#secret-vault-requests-fail-with-certificate-verify-failed).
- [`vault.ssl_verify`](/key-concepts/settings.md#vaultssl_verify) now applies to the connections the OpsChain API makes to the secret vault. It was honoured everywhere else and documented as taking effect, but the API ignored it, leaving no way to relax verification while diagnosing a certificate problem that was stopping the API from starting. The `ssl_verify` key in `vault.client_options` is honoured too, with `vault.ssl_verify` taking precedence over it.
- A certificate authority configured for the secret vault is now applied whether or not a client certificate is configured alongside it. Either one on its own was previously dropped.
- The OpsChain API worker now trusts the certificate authorities held in the trust store. It was the only OpsChain workload that did not mount them, so an authority you had added was trusted by the API but not by the worker that resolves secrets and starts your changes.
- Attempting to send an email while no default global email channel is configured now raises an `api:notification:error` event. Previously the failure was reported only to the caller, so an instance with no channel configured left nothing for an operator to look at when asking why no email was going out.
- A failed notification channel test send now raises an `api:notification:error` event describing the failure, as a failed real send already did. Repeated tests of the same channel are throttled, and are throttled separately from its real sends so that neither hides the other.
- An entry in a resource's `available_actions` now applies the options it carries and overrides nothing else. The entry's own `step_name:` was ignored, listing an action by name reset its `step_name`, `ignore_failure` and `reload_actions` to the defaults it was defined with, and an entry that renamed one action blanked the description of that action for every resource of the same type defined after it. See [overriding controller actions on a resource](/key-concepts/actions.md#overriding-controller-actions-on-a-resource).
- Once loading a change's `actions.rb` has failed, a later step of that change no longer fails with an unrelated `Errno::ECHILD` error. The failure that was reported for the step that could not load the file is now the only one reported.
- The OpsChain keywords can now be called from inside a resource definition, where they previously raised a `NoMethodError`. `query`, `secret_vault`, `context`, `dry_run?`, `opschain_cli`, `properties_for`, `repository_properties` and `repository_properties_for` join `log` in being available without the `OpsChain.` prefix, so a resource can work out its own properties from the MintModel generated for an asset, or from a credential held in the secret vault. `exec_command` and `send_email` are deliberately not among them. See [the OpsChain keywords in a resource definition](/key-concepts/actions.md#the-opschain-keywords-in-a-resource-definition).
- Filtering changes, steps and activities by `status_code` now returns the records whose displayed status is the one you asked for, and a change paused on a wait or input step that has steps nested under it is now listed as `waiting` rather than `running`. The filter worked the status out separately from the status shown on the record itself, and the two could disagree: `rejected` matched anything aborted while it had approvers, `waiting_for_approval` and `rejected` matched every waiting or aborted workflow run, `pending` missed the records waiting to be restarted, and `waiting` missed a change whose outstanding steps were all waiting. See [API filtering & sorting](/advanced/api-filtering.md) and [nesting child steps under a wait step](/key-concepts/actions.md#nesting-child-steps-under-a-wait-step).
- Supplying the reserved `opschain` metadata key when running a change or workflow run is now reported under the metadata editor. The request was already refused, with no reason given. See [change metadata](/key-concepts/changes.md#change-metadata).
- Signing in after a session expires now returns you to the page you were on, rather than to the dashboard.
- Dropdowns and popovers opened inside a dialog now close when you click outside them. They stayed open until the dialog itself was closed.
- A search field's list now shows a tick against the option that is selected.
- An optional input argument that nobody answered no longer stops a wait step continuing automatically. Where a step is eligible but cannot be continued, the reason is now written to the change log. See [automatically continuing wait steps](/key-concepts/actions.md#automatically-continuing-wait-steps).
- An input argument value that a step cannot accept is now refused by the form rather than sent with the change — a `hash` argument requires an object, and a `float` argument a number small enough to store.
- A Git remote whose server stops responding no longer starves the OpsChain workers. Every wait for exclusive access to a repository is now bounded by [`git_remote.fetch_lock_timeout`](/key-concepts/settings.md#git_remotefetch_lock_timeout), and an SSH connection to a Git server is given a connection timeout and keepalives so a session that has silently gone away is abandoned rather than waited on indefinitely. The same options are published to a step, so a [`git_clone`](/advanced/resource-types/index.md#opschain-git-clone) from a `url` uses them too. See the [`git_remote.ssh`](/key-concepts/settings.md#git_remotesshconnect_timeout) settings.
- A caller waiting behind another caller's fetch of a Git remote now fetches for itself when that fetch failed. The failed attempt was treated as having covered the waiter, so the waiter could carry on against a repository whose refs had never arrived. See [`git_remote.fetch_stale_threshold`](/key-concepts/settings.md#git_remotefetch_stale_threshold).
- Scheduled background jobs no longer accumulate while the queue they run on is not being worked. A job whose previous copy is still waiting is no longer enqueued again, so a stalled queue no longer builds a backlog that all becomes runnable at once when the workers restart.
- The queued step jobs of a change that was cancelled or errored are now removed. They sat on a queue only that change's runner pod would have consumed, so nothing worked, expired or deleted them.
- An asset whose template version has a MintModel now generates its actions when its `actions.rb` declares no actions of its own. The generation failed, although deleting the `actions.rb` outright had always succeeded and the MintModel supplies the asset's entire action catalog either way.
- An actions generation that reused the actions generated for an earlier request now records when those actions were originally generated. The request finished immediately with an empty log, which read as a request that had done nothing.
- A notification channel's credentials are now masked wherever they are echoed — the events raised when a channel is created, updated, deleted or tested, the OpsChain log, and the job payload the administration screen serves.
- A change's step tree now shows the steps that never ran as aborted, where the change or the step above them failed, was cancelled or was rejected before reaching them. They previously took the status of whatever stopped short of them.
- Each cleanup job run now reports the filters that run applied and how many agent images and job history rows it removed, and a job's card summarises the filters set against each of its options. An agent image cleanup was captioned with the change and workflow run filters instead. See [reviewing what a cleanup job has removed](/getting-started/familiarisation/gui/data_cleanup.md#reviewing-what-a-cleanup-job-has-removed).
- The agent images option on a cleanup job now warns when it has no filter set, as every other option already did, and an invalid filter is refused when the job is saved rather than when it runs. Left blank, it removes every unreferenced agent image. See [what a cleanup job can remove](/getting-started/familiarisation/gui/data_cleanup.md#what-a-cleanup-job-can-remove).
- A one-off cleanup job that has already run can no longer be renamed or rescheduled, so its history stays an accurate record of what it did.
- An agent running an outdated image now shows its own logs, rather than the Kubernetes events of its pod.
- Panning or scrolling a step tree no longer moves it out of the visible area.
- The error reported when a workflow cannot be created now reads the same way as the rest of the GUI's errors.
- The assets listed under a template version's linked assets now refresh when the list changes.
- Fixed an issue where an agent would fail to start because its ingress configuration looked for an inexistent service depending on the product name.
- An invalid cron expression is now reported in the form, and on the schedule's header and summary, rather than being submitted and refused by the API. See [setting a schedule](/getting-started/familiarisation/gui/scheduled_activities.md#setting-a-schedule).
- The value sent to the secrets endpoints is no longer written into the event the request raises, or into the OpsChain log. Storing, encrypting or resolving a secret recorded the value itself, where anyone able to read events could see it; the vault path, the owner and the fact that a value was supplied are still recorded.
- A secret written through the API is now stored at the path a change reads it back from. A path containing a hyphen reached the secret vault percent-encoded, so `MP-001` was stored as `MP%2D001` and a secret written through the API was invisible to the action code resolving the same path. Secrets already stored at an encoded path are moved by the `opschain:migrate_vault_secret_paths` task, which lists what it would move and only moves it when passed `apply`.
- OpsChain no longer keeps a hash of an LDAP user's directory password. One was written the first time that user signed in, could not be used to authenticate, and is cleared when you upgrade.
- Disabling a notification channel now stops it delivering. A webhook channel, or one that starts a change or workflow run, ignored the flag and kept firing; a disabled channel is now skipped and reports a `warn:notification_channel:disabled` event, and a subscriber can no longer be pointed at one.
- Deleting a project now removes the notification channels, filters and subscribers held against it. They were left behind, and because a filter matches on the node path, recreating a project at the same path revived the previous project's subscriptions and notified its subscribers about the new project. Rows already left behind are removed when you upgrade.
- Deleting a notification channel that cannot be deleted now says why. An internal channel, or one that still has subscribers, reported success while nothing was deleted.
- A notification whose channel is deleted or disabled before it is sent now reports an `api:notification:error` event, rather than being dropped with nothing recorded to say so.
- Deleting a certificate authority now removes it from the trust store. The certificate stayed mounted in every runner pod and trusted by every image built afterwards; upgrading reconciles the trust store against the certificates you have.
- A certificate uploaded from a Windows machine no longer counts as different content from the same certificate uploaded from Linux, which forced a runner image rebuild.
- In a [high availability deployment](/advanced/ha/index.md), each site now keeps its own copy of a Git remote up to date. A fetch at one site satisfied another site's freshness check, so a site could resolve a revision against a repository that had never received those refs, and the periodic fetch left every other site's copy cold.
- In a high availability deployment, a change is no longer aborted because its runner pod is at another site. The pod check ran against the local site only, so a change running elsewhere looked as though its pod had gone.
- A setting you change now takes effect everywhere. A cached copy of the global settings could be left holding the values from before your change indefinitely, and any node's settings worked out while it was stale kept those values too.
- An OpsChain worker pod deleted before it had finished starting now stops promptly, rather than staying `Terminating` for the whole termination grace period — an hour by default.
- A fresh install with cert-manager disabled no longer fails with `no matches for kind "Issuer"`. The self-signed issuer was rendered whether or not cert-manager was enabled, which is the install the [TLS guide](/setup/configuration/tls/index.md) offers for trialling OpsChain with its own certificates.
- Work held by a worker that was killed without shutting down is now picked up again within minutes rather than a couple of hours. PostgreSQL kept the dead connection's locks until its own keepalive detection expired, so the job stayed locked and unworkable by any live worker.
- A step whose logs overflow the log buffer again reports an `error:fluentd:log_failure` event, and that event now reaches notification subscriptions like any other. The lines are written straight to the database in that situation, and nothing said so.
- Fixed an intermittent failure where the first step of a change errored with `Server process exited with error code: 1` without having loaded `actions.rb`.
- An authorisation policy can now be assigned to a local user. The assignee was looked up in the LDAP directory whatever their account, so granting a policy to a locally created user failed saying the user could not be found. An LDAP account that has never signed in now has its OpsChain user created so the assignment takes effect immediately, and a username that neither OpsChain nor the directory holds is refused with a message naming it, rather than stored and left waiting for that person to sign in.
- Requesting a subset of a step's fields no longer returns a cached response built for a different set of fields. Two requests differing only in `fields[…]` shared a cache validator, so a client could be told its copy was current when it was not.
- A [`query`](/key-concepts/actions.md#querying-the-api) given a blank value for a node it identifies — an unset property, or an empty variable — now reports it, rather than dropping that segment of the path and failing as not found. Unsupported and blank keyword arguments are reported together.
- A cleanup job no longer purges the image of an agent build that has not finished, which could leave that agent unable to start.
