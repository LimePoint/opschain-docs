...
Add the OpsChain Helm chart repository to your Helm client:

```bash
helm repo add opschain https://limepoint.github.io/opschain-helm/
helm repo update
```

With your configured `values.yaml` and your configured namespace, we can install OpsChain in the desired Kubernetes namespace by running:

```bash
helm upgrade --install opschain opschain/opschain --create-namespace -n ${KUBERNETES_NAMESPACE} -f values.yaml --wait --timeout 30m --insecure-skip-tls-verify
```
