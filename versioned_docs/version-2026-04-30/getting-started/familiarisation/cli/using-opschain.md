...
Create a `.opschainrc` in your home directory (e.g. `~/.opschainrc` on Linux, macOS, or WSL) based on the [example](/files/config_file_examples/opschainrc.example) - be sure to update the `apiBaseUrl` to point to your OpsChain server installation, and the `username` and `password` configuration for your user account. On Windows - not within WSL - the configuration file should be placed in the `USERPROFILE` directory. See the [CLI configuration locations](/advanced/advanced-cli.md#cli-configuration-locations) guide if you would like to learn more.
...
If you would like to learn more about the CLI, see the [CLI reference guide](/advanced/advanced-cli.md).
...
:::tip
This setting can be overridden by specifying a `--project-code` explicitly on the command line. It can also be set in your [CLI configuration](/advanced/advanced-cli.md#opschain-cli-configuration-settings) if you are always working in the same project.
:::
