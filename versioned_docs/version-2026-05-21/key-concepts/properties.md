...
The root folder used to store the repository properties is configurable via the [OPSCHAIN_REPO_FOLDER](/setup/configuration/additional-settings.md#opschain_repo_folder) setting. We'll assume the default value of `opschain` for the rest of this guide.
...
Refer to the [CLI guide](/getting-started/familiarisation/cli/index.md) for more information on how to edit properties via the CLI.
...
Please resolve this conflict manually and correct the project properties via the GUI. If applicable, retry the change to complete any remaining steps.
```
...
If there are further steps in the change to run, and the failed step is idempotent, you can retry the change from the GUI to restart the change from the failed step. **It is important to note that OpsChain will re-run the failed step in its entirety.**
