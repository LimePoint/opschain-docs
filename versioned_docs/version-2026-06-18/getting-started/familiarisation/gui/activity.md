...
6. You can add properties overrides for the change using the _Properties overrides_ tab. These overrides apply only to this change and do not modify the target's stored properties.
7. You can add settings overrides for the change using the _Settings overrides_ tab. These overrides apply only to this change and do not modify the target's stored settings.
8. Also, you can add comments and other information in the _Metadata_ tab. You can later use the metadata to filter the activity table.
9. Confirm the creation by clicking the _Run change_ button.
