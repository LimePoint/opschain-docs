...
- A new `OpsChain.ignore_failure_step` helper is available for wrapping MintModel actions as child steps. It creates a thin wrapper action with `ignore_failure: true` so the change continues even if the MintModel step fails. See the [ignore failure steps](/key-concepts/actions.md#ignore-failure-steps) documentation for more information.
...
- The OpsChain pods now enforce `runAsNonRoot: true` and avoid entering the pod as non-root.
