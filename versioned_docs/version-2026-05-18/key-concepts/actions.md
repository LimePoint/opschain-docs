...
When your Git repository (and included `actions.rb`) are used by an [asset template](assets.md#asset-templates) the actions that have a description will be displayed in the GUI. To add a description to an action, include the `description:` option when defining the action, for example:
