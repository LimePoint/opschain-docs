...
5. If you'd like to schedule the change, you can do so by using the _Schedule_ tab. A change started partway through an action tree (via _Run \<action\> from here_) cannot be scheduled, so this tab is disabled in that case.
6. You can add custom notification settings for the change using the _Notify_ tab.
7. You can add properties overrides for the change using the _Properties overrides_ tab. These overrides apply only to this change and do not modify the target's stored properties. The editor provides JSON schema autocomplete and inline validation, suggesting keys and values and flagging invalid keys as you type.
...
7. You can add properties overrides for the workflow run using the _Properties overrides_ tab. These overrides apply only to this workflow run and do not modify the target's stored properties. The editor provides JSON schema autocomplete and inline validation, suggesting keys and values and flagging invalid keys as you type.
