...
If the agent repository contains a directory with a code matching the template code, then the `Dockerfile` in that directory will be used instead. For example, if the agent template was added to OpsChain with the code `sample_agent`, and the repository contains a file at `sample_agent/Dockerfile` then it will be used as the Dockerfile for this agent.
...
```dockerfile
COPY --chown opschain:opschain --link --from=repo --chmod=755 /sample_agent/agent.sh /opt/opschain/agent.sh
```

:::

See [the sample repository](https://github.com/LimePoint/opschain-getting-started/tree/agent-guide/sample_agent) for an example.
...
3. Create a new agent template using the created Git remote.
    a. Give it the code `sample_agent` to use the custom `Dockerfile` and `agent.sh` described [above](#agent-template-code-folder). A template's code must be unique within its project, so it cannot reuse the code of an existing asset template.
...
OpsChain agents can use the OpsChain API to create [OpsChain events](/key-concepts/events.md), which are surfaced on the [audit history page](/getting-started/familiarisation/gui/audit_history.md). This allows an OpsChain agent to use OpsChain event automations to trigger other OpsChain activities.
