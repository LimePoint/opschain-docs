...
OpsChain is designed to be backup and restore friendly, ensuring you can easily export and import your OpsChain instance in different environments. Most of the settings and configurations used by OpsChain reside encrypted within its database, making it the most important resource to back up on a regular basis.

There are two things to back up: your deployment configuration (the `values.yaml` file) and the OpsChain database. OpsChain can back the database up for you on a schedule. A database backup comes in one of two forms — a **physical** backup (a complete copy of the database files) or a **logical** backup (a portable export of its contents) — each suited to a different situation, as explained below. To restore an instance you need both the `values.yaml` file and a database backup.
...
Use **physical** backups for your regular scheduled backups and for disaster recovery — they are a complete, fast-to-restore copy of the database. Use a **logical** backup around upgrades — it can be restored into a newer PostgreSQL major version if needed. Keeping both a recent physical backup and a pre-upgrade logical backup covers the common cases.
:::
...
Restoring an instance has two parts: redeploy with the backed-up `values.yaml` file, then restore the database from a backup. Follow the path that matches the backup you hold — a [physical restore](#database-recovery) (from a a pre-deploy backup) or a [logical restore](#restoring-a-logical-backup) (from a scheduled backup). In a high availability setup, also review the [extra requirements for high availability clusters](/advanced/ha/operations.md#recovery-from-a-backup).
...
This section restores a **physical** backup (a `pg_basebackup`) by replacing the database's data files. For a **logical** backup (a `pg_dump`, such as a pre-deploy backup), follow [restoring a logical backup](#restoring-a-logical-backup) instead.
...
Once OpsChain is resumed, you can scale the secret vault back up to its configured replica count (`3` by default):
