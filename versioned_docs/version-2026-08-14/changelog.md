...
### Before upgrading {/* #unreleased-before-upgrading */}
...
The OpsChain secret vault now runs as several replicas. A request from OpsChain can land on a standby replica, which redirects the caller to the active replica at the secret vault external hostname (`global.secretVaultExternalHostName`). For that redirect to succeed, this hostname must resolve **from inside the Kubernetes cluster**, not only from client machines. If it does not, the OpsChain API can fail to start after upgrading, reporting that it cannot resolve the vault hostname.

Before upgrading, make the secret vault external hostname resolve from within the cluster — either through your organisation's DNS, or, on K3s, by creating or modifying a `coredns-custom` ConfigMap:
...
In a [high availability setup](/advanced/ha/index.md#external-service), each cluster's CoreDNS must resolve the external hostname of every cluster's secret vault. See [secret vault hostname](/setup/configuration/tls/index.md#secret-vault-hostname) for more detail.
:::
