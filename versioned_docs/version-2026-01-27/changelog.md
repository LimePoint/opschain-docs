...
- Changes (and scheduled changes) now support settings overrides. The [`Dockerfile` path](/key-concepts/settings.md#dockerfile), the base [`runner` image settings](/key-concepts/settings.md#runnerimage_tag), the [`worker` image settings](/key-concepts/settings.md#workerimage_tag), the [`mintmodel_executor` image settings](/key-concepts/settings.md#mintmodel_executorimage_tag), the [`pod_per_change_step` setting](/key-concepts/settings.md#pod_per_change_step), the [`remove_change_worker_pod` setting](/key-concepts/settings.md#remove_change_worker_pod), and the [`repo_folder`](/key-concepts/settings.md#repo_folder) can now be overridden for a change.
