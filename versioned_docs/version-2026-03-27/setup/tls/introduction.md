...
Additionally, when enabling external services for the secret vault or the [database (for high availability setups)](/advanced/high-availability-setup.md#external-service), these ports will need to be opened on the firewall to allow incoming connections from the other instances.
