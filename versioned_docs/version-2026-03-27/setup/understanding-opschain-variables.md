...
Modifying any of these settings after installation will result in data loss. When deploying in high availability mode, you must set this value to the same value across all instances.
...
To support a [high availability setup](/advanced/high-availability-setup.md) using the default secret vault, you must configure its service to allow incoming connections from the other instances:
