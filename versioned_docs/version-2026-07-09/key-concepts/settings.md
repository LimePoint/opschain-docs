...
OpsChain runs each change worker, step runner and actions refresh in its own Kubernetes pod. On busy clusters this can create more pods than the cluster can comfortably schedule at once. These settings cap how many runner and actions-refresh pods OpsChain will run concurrently on a cluster. When a cap is reached, further pods wait and are admitted as running pods complete.

The two caps share capacity: if one pool is not using its full allocation, the other may borrow the spare slots, but the combined number of running pods never exceeds the sum of the two limits.
