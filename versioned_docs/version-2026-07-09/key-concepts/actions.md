...
`OpsChain.step` is a convenience helper that creates a wrapper step combining wait, input, and ignore failure behaviours in a single call. It takes the name of an existing action and one or more options:

| Option | Default | Description |
|---|---|---|
| `ignore_failure:` | `false` | If `true`, the change will continue even if the wrapped action fails. |
| `wait:` | `false` | If `true`, a wait step is prepended — the change pauses for manual continuation before running the wrapped action. Pass an integer to sleep for that many seconds instead. |
| `input_arguments:` | `nil` | Prepends an input step — the change pauses and prompts the user for input before running the wrapped action. Mutually exclusive with `wait:`. |
| `step_name:` | `nil` | A custom display name for the outer wrapper step. |
| `wait_step_name:` | `nil` | A custom display name for the inner wait or input step. |
...
In this example the change will pause at a wait step before running `Shutdown`. If `Shutdown` fails, the change continues to `infra_destroy`.
...
The wrapper step name is derived from the wrapped action name and the active options, for example `Run "Shutdown" waiting for explicit continue, ignoring failures`. Subsequent calls with the same action name and options append an incrementing suffix (`Run "Shutdown" waiting for explicit continue, ignoring failures 2`, etc.).
