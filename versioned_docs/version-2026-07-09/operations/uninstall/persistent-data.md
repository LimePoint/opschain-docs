...
If you enabled [automated database backups](/operations/maintenance/backups.md#automated-backups), the `opschain-db-backup` persistent volume claim is deliberately retained on uninstall so that your database backups are not lost. It is also preserved when a cluster changes role during failover or failback.

If you have captured your backups elsewhere and want to remove the volume, delete the claim:

```bash
kubectl -n ${KUBERNETES_NAMESPACE} delete pvc opschain-db-backup
```

:::warning
Deleting this volume permanently removes every database backup stored on it. Make sure you have copied any backups you need to a safe location first.
:::
