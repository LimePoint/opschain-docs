...
:::tip[Resolving cross-cluster hostnames inside the cluster]
Each cluster reaches its peers by hostname (for example the external database or secret vault of another cluster), and those names must resolve from inside the cluster.

If these hostnames are not already served by your organisation's DNS, you can add them to K3s' CoreDNS with a `coredns-custom` ConfigMap. Use a dedicated server block per name so it does not clash with CoreDNS' built-in configuration (an inline `hosts` block placed in the default server block will conflict with the entries K3s already manages there). For example:
...
data:
  opschain-peers.server: |
    peer-cluster.example.com {
      hosts {
        10.0.0.10 peer-cluster.example.com
        fallthrough
      }
    }
```
...
Confirm CoreDNS comes back healthy (`kubectl -n kube-system get pods -l k8s-app=kube-dns`) and that the name resolves before redeploying OpsChain. The peer's TLS certificate must also list the hostname — see the [`serverAltDNSNames`](#certificates) setting above.
:::
